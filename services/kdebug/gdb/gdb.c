...
hex2mem(char *buf, char *mem, int count) {
	...
	case 4:
		if(MAP_IN_RANGE(inout, mem)) {
			out16(MAP_OFFSET(inout, mem), temp.u32);
		} else {
...
gdb_mapping_del(void *p, size_t len) {
	if(!MAP_IN_RANGE(inout, p) && !MAP_IN_RANGE(direct, p)) {
		mapping_del(p, len);
	}
}
...
gdb_interface(struct kdebug_entry *entry, CPU_REGISTERS *ctx, ulong_t sigcode) {
	...
	if(!connected) {
		...
		if(entry && (kinfo = private->kdebug_info)) {
			char				buff[_POSIX_PATH_MAX + 1];

			kinfo->debug_path(entry, buff, sizeof buff);
			kprintf(" in %s", buff);
		}
		if(debug_flag > 1) {
