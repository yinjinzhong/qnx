...
int mempartmgr_link(resmgr_context_t *ctp, io_link_t *msg, void *attr, io_link_extra_t *extra)
{
	switch (msg->connect.extra_type)
	{
		case _IO_CONNECT_EXTRA_SYMLINK:
		{
			...
			int  r;
			mempartmgr_attr_t  *mp, *mp_parent, *mp_real;
...
//FIX ME - check extra_len first
			...
			if ((mp_real = path_find((mempartmgr_attr_t *)LIST_FIRST(root_mpart->children), chk_part_name, NULL)) == NULL)
				return ENOENT;
/*
			mp_parent = NULL;
			if ((mp = path_find((mempartmgr_attr_t *)LIST_FIRST(root_mpart->children), pseudo_name, &mp_parent)) != NULL)
				return EEXIST;
*/
			...
			while(1)
			{
				...
				mp = path_find((mempartmgr_attr_t *)LIST_FIRST(mp_parent->children), chk_part_name, NULL);
			...
			if ((mp_parent->type == mpart_type_MEMPART_real) ||
				(mp_parent->type == mpart_type_MEMPART_pseudo))
			{
			...
			if (msg->connect.ioflag & O_CREAT)
			{
				...
				mp->type = mpart_type_MEMPART_pseudo;
				...
				mp->mpid = (mempart_id_t)strdup(part_name);
...
__SRCVERSION("$IQ: mempartmgr_link.c,v 1.23 $");
