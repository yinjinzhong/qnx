 ...
 * find_pidentry
 * 
 * Return the pid_attr_entry_t for <pid> or NULL if it does not exist
*/
/* FIX ME - this function has been deprecated as it it no longer used
 * I leave it here as reference for now ... just in case
pid_attr_entry_t *find_pidentry(pid_t pid)
{
	pid_attr_entry_t  *p = (pid_attr_entry_t *)LIST_FIRST(pid_entry_list);

	while(p != NULL)
	{
		if (p->pid_attr.pid == pid)
			break;
		else
			p = LIST_NEXT(p);
	}
...
bool isProcessPath(char *path, pid_t *pid_p)
{
	...
	KASSERT((path != NULL) && (*path != '\0') && (pid_p != NULL));
 ...
 * argument <partmgrtype> is used to indicate which resource manager is making
...
*/
bool nametoolong(const char *name, unsigned chktype, void *partmgrtype)
{
	unsigned prefix_len;

#ifndef NDEBUG
// FIX ME
extern dev_t schedpartmgr_devno;
	if (name == NULL) crash();
	if ((chktype != PATH_MAX) && (chktype != NAME_MAX)) crash();
	if ((partmgrtype != (void *)partmgr_devno) &&
		(partmgrtype != (void *)mempartmgr_devno) &&
		(partmgrtype != (void *)schedpartmgr_devno)) crash();
#endif	/* NDEBUG */

	prefix_len = (sizeof("/proc/") - 1) +
				 MAX_PID_T_DIGITS +
				 (sizeof("/partition/") - 1) +
				 /* for partmgr_devno and schedpartmgr_devno, we use "sched/", the longest */
				 (((partmgrtype == (void *)mempartmgr_devno) ? sizeof("mem/") : sizeof("sched/")) - 1);
...
__SRCVERSION("$IQ: partmgr_misc.c,v 1.23 $");
