 ...
 * 
*/
int mempartmgr_readlink(resmgr_context_t *ctp, io_readlink_t *msg, RESMGR_HANDLE_T *handle, void *reserved)
{
	mempartmgr_attr_t  *mp;
	int  r;

	if ((mp = path_find((mempartmgr_attr_t *)LIST_FIRST(root_mpart->children), msg->connect.path, NULL)) == NULL)
		return ENOENT;

	if ((r = MEMPART_ATTR_LOCK(mp)) != EOK)
		return r;

	if (S_ISLNK(mp->attr.mode) && (mp->type == mpart_type_MEMPART_pseudo))
	{
		mempartmgr_attr_t *mp_real;
		char *link_name = (char *)mp->mpid;
		...
		if ((mp_real = path_find((mempartmgr_attr_t *)LIST_FIRST(root_mpart->children), link_name, NULL)) == NULL)
			r = ENOLINK;
		else if (mp_real->type != mpart_type_MEMPART_real)
			r = EBADF;
		else
		{
			...
			STRLCPY(link_string, (char *)mp->mpid, space);
...
__SRCVERSION("$IQ: mempartmgr_readlink.c,v 1.23 $");
