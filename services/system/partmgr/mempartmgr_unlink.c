...
int mempartmgr_unlink(resmgr_context_t *ctp, io_unlink_t *msg, RESMGR_HANDLE_T *handle, void *reserved)
{
	switch (msg->connect.subtype)
	{
		...
		{
			int  r;
			mempartmgr_attr_t *mp_parent = root_mpart;
			mempartmgr_attr_t *mp = path_find((mempartmgr_attr_t *)LIST_FIRST(root_mpart->children), msg->connect.path, &mp_parent);
			...
			*/
			...
			if (mp->type == mpart_type_MEMPART_real)
			{
				...
				mempart_info_t *meminfo = MEMPART_GETINFO(mp->mpid, &tmp_info);

				KASSERT(meminfo != NULL);
			...
			*/
			...
			KASSERT(mp->parent != NULL);
			KASSERT(LIST_FIRST(mp_parent->children) != NULL);
			KASSERT(mp->attr.nlink >= (S_ISDIR(mp->attr.mode) ? 2 : 1));
			KASSERT(mp_parent->attr.nlink >= 2);
			...
			*/
			if (((r = iofunc_client_info(ctp, msg->connect.ioflag, &ci)) != EOK) ||
				((r = check_access_perms(ctp, mp, S_IWUSR | S_IWGRP | S_IWOTH, &ci, bool_t_FALSE)) != EOK) ||
				((r = check_access_perms(ctp, mp_parent, S_IWRITE, &ci, bool_t_FALSE)) != EOK))
			{
				MEMPART_ATTR_UNLOCK(mp);
				MEMPART_ATTR_UNLOCK(mp_parent);
				return r;
			...
			switch (mp->type)
			{
				...
				{
					memclass_entry_t * mce = memclass_find(NULL, (memclass_id_t)mp->mpid);
					...
					memclass_delete(NULL, mce->data.info.id);
				...
				{
					KASSERT(mp->mpid != mempart_id_t_INVALID);

					/* can't delete the system ram memory partition */
					if (mp->mpid == MEMPART_T_TO_ID(sys_mempart))
					{
						MEMPART_ATTR_UNLOCK(mp);
						MEMPART_ATTR_UNLOCK(mp_parent);
						return EPERM;
					...
					if ((LIST_FIRST((MEMPART_ID_TO_T(mp->mpid))->prp_list) != NULL) ||
						(LIST_FIRST((MEMPART_ID_TO_T(mp->mpid))->obj_list) != NULL))
					{
						MEMPART_ATTR_UNLOCK(mp);
						MEMPART_ATTR_UNLOCK(mp_parent);
						return EBUSY;
					...
					if ((r = MEMPART_DESTROY(mp->mpid)) != EOK)
					{
						MEMPART_ATTR_UNLOCK(mp);
						MEMPART_ATTR_UNLOCK(mp_parent);
						return r;
				...
				case mpart_type_MEMPART_pseudo:
					free((char *)mp->mpid);	// symlink contents
					break;
...
__SRCVERSION("$IQ: mempartmgr_unlink.c,v 1.23 $");
