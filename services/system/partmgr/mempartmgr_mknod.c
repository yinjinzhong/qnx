 ...
 * 
*/
int mempartmgr_mknod(resmgr_context_t *ctp, io_mknod_t *msg, RESMGR_HANDLE_T *handle, void *reserved)
{
	switch (msg->connect.subtype)
	{
		case _IO_CONNECT_MKNOD:
		{
			...
			r = mempartmgr_open(ctp, (io_open_t *)msg, handle, reserved);
...
__SRCVERSION("$IQ: mempartmgr_mknod.c,v 1.23 $");
