...
#include <kernel/hooks.h>
	...
	#pragma aux intrevent_drain_check	modify exact [eax ebx ecx edx esi edi gs];
...
int           rdecl signal_discard(PULSE **queue, PROCESS *prp, THREAD *thp, int signo);
int           rdecl signal_clear_thread(THREAD *thp, siginfo_t *sip);
...
int					kvsnprintf(char *buf, size_t buflen, const char *fmt, __va_list ap);
...
SYNC *        rdecl sync_create(sync_t *sync, unsigned flags);
...
PULSE *       rdecl pulse_add(PROCESS *prp, void *queue, int priority, int code, int value, int id);
PULSE *       rdecl pulse_replace(PROCESS *prp, void *queue, int priority, int code, int value, int id);
...
int           rdecl intrevent_alloc(int num, int init);
...
void		  intrevent_drain_attr intrevent_drain_check(void);
...
/* __SRCVERSION("proto.h $Rev: 156323 $"); */
