 ...
 */
#define LIBMOD_VERSION_CHECK	44	/* trinity 2 */ 
...
#define _NTO_TF_KERERR_LOCK		0x00040000
...
#define SIGCODE_SSTEP				0x80000000 /* for use with F.P. emulator */
	...
	#define WITHIN_BOUNDRY(first, last, boundry) ((last) <= (boundry))
#else
	// kernel/ProcNto is in low memory
	#define WITHIN_BOUNDRY(first, last, boundry) ((first) >= (boundry))
#endif
...
#define LINK1_END(_queue, _object, _type) {\
		_type **_prev;\
		(_object)->next = 0;\
		for(_prev = &(_queue); *_prev; _prev = &((*_prev)->next));\
		*_prev = (_object);\
		}

#define LINK1_SORTLE(_queue, _object, _type, _sort) {\
		_type **_prev;\
		for(_prev = &(_queue); *_prev && \
				(*_prev)->_sort <= (_object)->_sort; \
				_prev = &((*_prev)->next));\
		(_object)->next = *_prev;\
		*_prev = (_object);\
		}

#define LINK1_SORTGE(_queue, _object, _type, _sort) {\
		_type **_prev;\
		for(_prev = &(_queue); *_prev && \
				(*_prev)->_sort >= (_object)->_sort; \
				_prev = &((*_prev)->next));\
...
#define LINK1_PRI(_queue, _object, _type) \
		LINK1_SORTGE(_queue, _object, _type, priority);
...
#define LINK2_PREV(_object, _type) \
		((_type *)((((char *)((_object)->prev))) - offsetof(_type, next)))

#define LINK2_NEXT(_object, _type) \
		((_object)->next)
...
#define LINK2_END(_queue, _object, _type) {\
		_type **_prev;\
		for(_prev = &(_queue) ; *_prev ; _prev = &((*_prev)->next));\
		LINK2_BEG(*_prev, _object, _type);\
		}

#define LINK2_SORTLE(_queue, _object, _type, _sort) {\
		_type **_prev;\
		for(_prev = &(_queue);\
				*_prev && (*_prev)->_sort <= (_object)->_sort;\
				_prev = &((*_prev)->next));\
		LINK2_BEG(*_prev, _object, _type);\
		}

#define LINK2_SORTGE(_queue, _object, _type, _sort) {\
		_type **_prev;\
		for(_prev = &(_queue);\
				*_prev && (*_prev)->_sort >= (_object)->_sort;\
				_prev = &((*_prev)->next));\
...
#define LINK2_SORTL(_queue, _object, _type, _sort) {\
		_type **_prev;\
		for(_prev = &(_queue);\
				*_prev && (*_prev)->_sort < (_object)->_sort;\
				_prev = &((*_prev)->next));\
...
#define LINK2_SORTG(_queue, _object, _type, _sort) {\
		_type **_prev;\
		for(_prev = &(_queue);\
				*_prev && (*_prev)->_sort > (_object)->_sort;\
				_prev = &((*_prev)->next));\
...
#define LINK2_PRI(_queue, _object, _type) \
		LINK2_SORTGE(_queue, _object, _type, priority);
...
#define KILO(x)			((x) * 1024)
	...
	#define KASSERT(e)		if (!(e)) crash()
#endif

/* __SRCVERSION("macros.h $Rev: 156323 $"); */
