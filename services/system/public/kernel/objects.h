...
struct process_entry {
	...
	PROCESS							*guardian;
	struct procmgr_event_entry		*event;
	uint64_t						 start_time;
...
struct pulse_entry {
	...
	PULSE			**prev;
	uint8_t			 type;
	uint8_t			 priority;	// May be a signal number for TYPE_SIGNAL
	uint8_t			 spare;
	int8_t			 code;
	uint32_t		 value;
...
struct syncevent_entry {
	...
	struct syncevent_entry **prev;
	uint8_t			 type;
	uint8_t			 priority;	/* position of type and priority field should be same as thread_entry */
	uint8_t			 subtype;
	uint8_t			 reserved;
	union {
		int 		 clockid;
...
/* __SRCVERSION("objects.h $Rev: 156713 $"); */
