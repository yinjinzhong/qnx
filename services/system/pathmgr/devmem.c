...
mem_close_ocb(resmgr_context_t *ctp, void *reserved, void *vocb) {
	...
	if(ocb) {
		...
		if((obp = ocb->object)) {
			//RUSH3: get rid of OBJECT_SHM_ADJ_REF_COUNT and
			//RUSH3: have a generic memobj_invisible()? OBJECT_MEM_SHARED
			//RUSH3: has to check that obp->hdr.node == NULL and in the
			//RUSH3: shared mem unlink code we need to be able to find
			//RUSH3: out if there are any open OCB's for the object.
			switch(obp->hdr.type) {
...
mem_stat(resmgr_context_t *ctp, io_stat_t *msg, void *vocb) {
	...
	if(ocb && obp) {
		...
		msg->o.st_nlink = OBJECT_SHM_REF_COUNT(obp);
		...
		msg->o.st_dev = ctp->info.srcnd | mem_devno;
		msg->o.st_rdev = 0;
	}
	return _RESMGR_PTR(ctp, &msg->o, sizeof msg->o);
}
...
mem_open(resmgr_context_t *ctp, io_open_t *msg, void *handle, void *extra) {
	...
	if((ocb->object = object)) {
		OBJECT_SHM_ADJ_REF_COUNT(object, 1);
	}
...
#define ALL_TMEM	(POSIX_TYPED_MEM_ALLOCATE|POSIX_TYPED_MEM_ALLOCATE_CONTIG|POSIX_TYPED_MEM_MAP_ALLOCATABLE)
	...
	errno = memmgr_tymem_open(msg->connect.path, &ocb->object, proc_lookup_pid(ctp->info.pid));
	...
	OBJECT_SHM_ADJ_REF_COUNT(ocb->object, 1);
	return EOK;
...
__SRCVERSION("devmem.c $Rev: 157074 $");
