...
pathmgr_resmgrlink(resmgr_context_t *ctp, io_link_t *msg, void *handle, struct _io_resmgr_link_extra *extra) {
	...
	if(!(obp = pathmgr_object_attach(0, msg->connect.path, OBJECT_SERVER, extra->flags, extra))) {
		return ENOMEM;
	}
...
pathmgr_sys_symlink(resmgr_context_t *ctp, io_link_t *msg, void *handle, char *path) {
	...
	if(!(obp = pathmgr_object_attach(0, msg->connect.path, OBJECT_PROC_SYMLINK, 0, path))) {
		return ENOMEM;
	}
	return EOK;
}
...
/*
 ...
 if path is too long to put in the buffer, return -1
*/
int 
pathmgr_node2fullpath(NODE* node, char *path, int pathmax) {
	...
	/* If local path requested or if netmgr fails, don't include network component */
		if(	(len = netmgr_ndtostr(ND2S_DIR_SHOW,
				node->object?node->object->server.nd:ND_LOCAL_NODE, path, pathmax)) <= 1) {
			len = 1;
			*path = '\0';
		} else {
			if(path[len - 2] == '/') {
		...
		do {
			for(n = node; n != n->parent; n = n->parent) {
				if(n->parent == prev || n->parent == node) {
					break;
				}
			}
			if(n->len) {
				...
				len += straddstr("/", 0, &path, (size_t*)&pathmax);
				len += straddstr(n->name, n->len, &path, (size_t*)&pathmax);
			}
			prev = n;
		} while(n != node);
