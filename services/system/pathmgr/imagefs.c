...
imagefs_mount(paddr_t paddr, unsigned size, unsigned offset, unsigned flags, struct node_entry *root, 
	...
	obp = object_create(OBJECT_MEM_SHARED, &msg, mempart_getid(NULL, sys_memclass_id));
	if(obp == NULL) {
		r = EAGAIN;
		goto fail1;
	}
...
fail3:	
	(void)memmgr.munmap(NULL, (uintptr_t)addr, size, 0, obp->hdr.mpid);
...
__SRCVERSION("imagefs.c $Rev: 156323 $");
