 		...
 		if (ND_NODE_CMP(ND_LOCAL_NODE, ctp->info.nd) != 0) {
			...
			struct _procfs_map_info mi;
 			...
 			while(1) {
				...
				if ( mappedsize == (unsigned)-1 || !(mi.flags & PROT_READ) ) {
					ProcessBind(0);
					proc_unlock_adp(prp);
					proc_unlock(prp);
					return EINVAL;
...
static int procfs_stat(resmgr_context_t *ctp, io_stat_t *msg, void *vocb) {
	...
	msg->o.st_dev = (ctp->info.srcnd << 16) | procfs_devno;
	msg->o.st_rdev = 0;
	...
	default:
		...
		case DCMD_PROC_MAPDEBUG_BASE:
			...
			if ( prp->debug_name != NULL ) {
				STRLCPY(ioctl->mapdebug.path, prp->debug_name, msg->i.nbytes - ((size_t)ioctl->mapdebug.path - (size_t)msg));
				msg->o.ret_val = 0;
				SETIOV(ctp->iov + 0, &msg->o, sizeof msg->o + offsetof(struct _procfs_debug_info, path) + strlen(ioctl->mapdebug.path) + 1);
		...
		case DCMD_PROC_MAPDEBUG:
			...
			if(memmgr.mapinfo(prp, ioctl->mapdebug.vaddr, NULL, &ioctl->mapdebug,
													msg->i.nbytes - ((size_t)ioctl->mapdebug.path - (size_t)msg),
													NULL, NULL, NULL) == 0)
			{
				proc_unlock_adp(prp);
				return proc_error(ENXIO, prp);
	...
	((chp = QueryObject(_QUERY_PROCESS, prp->pid, _QUERY_PROCESS_CHANCONS, id, &id, &chan, sizeof(chan))) != NULL )) {
		...
		case DCMD_PROC_GET_MEMPART_LIST:
		{
			...
			*/
			...
			ret_val = nbytes = MEMPART_LIST_T_SIZE(ioctl->mpart_list.num_entries);
			if (r > 0) {
		...
		case DCMD_PROC_DEL_MEMPARTID:
		{
			...
			if ((r = MEMPART_VALIDATE_ID(ioctl->mempart_id)) == EOK)
			{
				...
				else
				{
					...
					if ((r = ProcessAssociate(prp, ioctl->mempart_id, mempart_flags_NONE)) == EALREADY)
						r = EOK;
				}
			}
		...
		*/
		case DCMD_PROC_CHG_MEMPARTID:
		{
			...
			if ((r = MEMPART_VALIDATE_ID(ioctl->mempart_id)) == EOK)
			{
				...
				r = ProcessReassociate(prp, ioctl->mempart_id, mempart_flags_NONE);
			}
			return proc_error(r, prp);
		}
		...
		if (pid) {
			...
			if(flags & PROCFS_FLAG_AS) {
				...
				if (mode) {
					...
					iofunc_attr_init(&attr, S_IFREG|0644, NULL, NULL);
					attr.uid = prp->cred->info.ruid;
					attr.gid = prp->cred->info.rgid;
...
__SRCVERSION("procfs.c $Rev: 156323 $");
