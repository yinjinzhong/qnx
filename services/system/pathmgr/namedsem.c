 ...
 */
static void detail_sem(semaphore_t *sem, struct stat *st, int namespace)
{
 ...
 */
static int list_sem(semaphore_t *dir, off64_t *offset, struct dirent *dp, int dpsize, int xtra, int *nbytes)
{
semaphore_t					*sem;
	...
	*nbytes = 0;
	for (;;) {
		...
		if (xtra && dpsize >= ((dp->d_reclen + sizeof(struct dirent_extra_stat) + 7) & ~7)) {
			...
			detail_sem(sem, &dextra->d_stat, !0);
...
{
	detail_sem(OCB2SEM(ocb), &msg->o, 0);
...
struct _msg_info	info;
	...
	if (MsgInfo(ctp->rcvid, &info) != -1 && (info.flags & _NTO_MI_UNBLOCK_REQ)) {
		_mutex_lock(&SemaphoreMutex);
		unblock_clients(OCB2SEM(ocb), ctp, !0, EINTR);
		_mutex_unlock(&SemaphoreMutex);
	}
	return(_RESMGR_NOREPLY);
}
...
int					error, xtra, nbytes;
	...
	_mutex_lock(&SemaphoreMutex);
	error = list_sem(OCB2SEM(ocb), &ocb->offset, (struct dirent *)msg, min(msg->i.nbytes, ctp->msg_max_size - ctp->offset), xtra, &nbytes);
	_mutex_unlock(&SemaphoreMutex);
...
__SRCVERSION("namedsem.c $Rev: 153052 $");
