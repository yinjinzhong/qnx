...
static void trace(PROCESS *prp, siginfo_t *info) {
	...
	if(info->si_code > SI_USER) {
		struct _procfs_debug_info	*debuginfo = (struct _procfs_debug_info *)buff;
		...
		case SIGBUS:
			proc_rlock_adp(prp);
			(void) memmgr.mapinfo(prp, (uintptr_t)vaddr, NULL, debuginfo,
												sizeof(buff) - offsetof(struct _procfs_debug_info, path), NULL, NULL, NULL);
			proc_unlock_adp(prp);
	...
	if(info->si_signo == SIGCHLD) {
		...
		if(info->si_signo > sizeof sigstrs / sizeof *sigstrs) {
			p += ksnprintf(p, (&buff[sizeof buff] - p) - 2, "signo=%d", info->si_signo);
		} else {
			p += ksnprintf(p, (&buff[sizeof buff] - p) - 2, "SIG%s", sigstrs[info->si_signo - 1]);
		}
...
static void cleanup(PROCESS *prp, siginfo_t *info) {
	...
	if(!exec && prp->alarm) {
		...
		(void) TimerAlarm(CLOCK_REALTIME, &cancel, NULL);
	}
	...
	while((chp = QueryObject(_QUERY_PROCESS, prp->pid, _QUERY_PROCESS_CHANCONS, ++i, &i, 0, 0))) {
		if(chp->type == TYPE_CONNECTION) {
			ConnectDetach(_NTO_SIDE_CHANNEL | i);
		}
	}

	//
	// Run the loop again because you could have a server connection id 
	// that is lower than the connection id and you need to close both.
	//
	// E.g.:
	//	  ConnectAttach(0, 0, ChannelCreate(0), _NTO_SIDE_CHANNEL, 0);
	//
	// The scoid will be lower than the coid so the first loop will try to
...
void *terminator(void *parm) {
	...
	if((lcp->state != LC_EXEC_SWAP) && prp->debug_name) {
...
__SRCVERSION("proc_termer.c $Rev: 153052 $");
