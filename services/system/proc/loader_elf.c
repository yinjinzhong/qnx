...
//RUSH3: Don't need this function anymore - mprotect allows unaligned parms again
static int
rounded_mprotect(uintptr_t vaddr, size_t len, int prot) {
	return mprotect((void *)(vaddr & ~(__PAGESIZE-1)), len + (vaddr & (__PAGESIZE-1)), prot);
}
...
elf_load(int fd, const char *path, struct loader_startup *lsp, struct stat *statlocal, struct inheritance *parms) {
		...
		for(i = 0; i < ehdr.e_phnum; i++) {
			...
			if((seg[i].size != 0) && seg[i].read_in) {
				if((seg[i].prot & PROT_WRITE) == 0) {
					rounded_mprotect(seg[i].addr, seg[i].size, seg[i].prot);
				}
	...
	//RUSH1: This mprotect is always going to fail - the flags will get error
	//RUSH1: checked out. This (and the following one) is to set the 'reloc'
	//RUSH1: field of the mm_mmap structure - do we still even need that?
	if((debug_msg.i.text_reloc = seg[text_seg].reloc)) {
		rounded_mprotect(seg[text_seg].addr, seg[text_seg].reloc, -162534);
	}
	debug_msg.i.data_addr = seg[data_seg].addr;
	debug_msg.i.data_size = seg[data_seg].size;
	if((debug_msg.i.data_reloc = seg[data_seg].reloc)) {
		rounded_mprotect(seg[data_seg].addr, seg[data_seg].reloc, -162534);
	}
...
__SRCVERSION("loader_elf.c $Rev: 154853 $");
