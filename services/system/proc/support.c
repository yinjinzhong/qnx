...
static int rdecl
aquire_lock(struct proc_mux_lock **lpp) {
	...
	if(lp == NULL) {
		...
		if(lp != NULL) {
			...
			(void)pthread_mutexattr_setrecursive(&attr, 1);
			(void)pthread_mutex_init(&lp->mux, &attr);
	...
	*lpp = lp;
	lp->requestors++;
	_mutex_unlock(&mux_mutex);
	status = pthread_mutex_lock(&lp->mux);
#ifndef NDEBUG
	if(status != EOK) {
		crash();
	}
...
release_lock(struct proc_mux_lock **lpp) {
	...
	if(lp == NULL) crash();
#endif
	status = pthread_mutex_unlock(&lp->mux);
#ifndef NDEBUG
	if(status != EOK) {
		crash();
	}
...
int rdecl
proc_mux_haslock(struct proc_mux_lock **mp) {
	int		hasit;

	hasit = 0;
	_mutex_lock(&mux_mutex);
	if(*mp != NULL) {
		if(((*mp)->mux.__owner & ~_NTO_SYNC_WAITING) == __tls()->__owner) {
			hasit = 1;
		}
	}
	_mutex_unlock(&mux_mutex);
...
__SRCVERSION("support.c $Rev: 153052 $");
