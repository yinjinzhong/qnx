...
OBJECT *pathmgr_object_attach(NODE *nop, const char *path, int type, unsigned flags, void *data);
void pathmgr_object_detach(OBJECT *obp);

OBJECT *object_create(int type, void *extra, mempart_id_t mpid);
int object_done(OBJECT *);
size_t object_name(OBJECT *, size_t, char *);
int object_devctl(resmgr_context_t *, io_devctl_t *, OBJECT *);

/* __SRCVERSION("pathmgr_proto.h $Rev: 156323 $"); */
