...
#define MAX_MASK		(128-1)
	...
	#define TICKER_START()	(timers_ticking = 1)
	#define TICKER_STOP()	(timers_ticking = 0)
	#define TICKER_WAIT()	do { __cpu_membarrier(); } while(timers_ticking)
#else
	#define TICKER_START()	
	#define TICKER_STOP()	
	#define TICKER_WAIT()	
...
#define KEROP_STOP()	(timers_kerop = 0)
...
static void
timer_insert(TIMER *tip, INTERRUPT *isr) {
	TIMER				*fore;
	TIMER				*back;
...
static void
timer_rearm(TIMER *tip, INTERRUPT *isr) {
	uint64_t	interval;
	QTIME		*qtp;
	...
	*/
	...
	timer_insert(tip, isr);
}


static TIMER *
timer_fire(TIMER *tip, INTERRUPT *isr) {
	THREAD		*thp;
	TIMER		*next;
	...
	CRASHCHECK(tip->link.next == NULL);
		
	if(timers_kerop && (isr != NULL)) {
	...
	// If there is no thread associated with the timer, find one
	if(tip->tid == -1) {
		thp = tip->process->valid_thp;
	} else {
		thp = vector_lookup(&tip->process->threads, tip->tid);
	}
	...
	if(thp != NULL) {
		overrun = 0;
		if(isr != NULL) {
			intrevent_add(&tip->event, thp, isr);
		} else {
			sigevent_exe(&tip->event, thp, 1);
		}
	...
	if((tip->pending == NULL) && (tip->queue != NULL)) {
		...
		if(prev->link.next == tip) {
			...
			// to fire it again. Eventually it'll be after...
			next = tip;
		}
	}
	return next;
		...
		if(queue->num_active != 0) {
			...
			for( ;; ) {
				...
				for( ;; ) {
					...
					//
					if((++nfires > 50) || (queued_intrevents > grow_intrevents)) {
...
timer_next(uint64_t *np) {
	...
	} while(tod_adj != qtimeptr->nsec_tod_adjust);
...
__SRCVERSION("nano_timer.c $Rev: 153052 $");
