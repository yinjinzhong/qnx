...
cpu_perfreg_id(void) 
{
	...
	unsigned cpu, imp_number, company_id;
	...
	cpu = getcp0_prid();
	imp_number = MIPS_PRID_IMPL(cpu);
	company_id = MIPS_PRID_COMPANY(cpu);

	if ( company_id ) {
		if(company_id == MIPS_PRID_COMPANY_BROADCOM) {
			if ( imp_number == MIPS_PRID_IMPL_SB1 ) {
