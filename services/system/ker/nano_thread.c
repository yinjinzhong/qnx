...
#define chk_lock()	;
#else
#define chk_lock() if((get_inkernel() & (INKERNEL_NOW | INKERNEL_LOCK)) == INKERNEL_NOW) crash();
#endif
//
...
chk_lock();
	...
	*/
	...
	if(thread_create_flags & THREAD_CREATE_BLOCK_FLAG) {
		// If the parent needs to know if the create succedded, we block it.
...
#ifndef NDEBUG
if(syp->waiting->args.mu.owner == 0) crash();
#endif
...
crash();
			...
			(void)memmgr.munmap(NULL, (uintptr_t)thp->un.lcl.stackaddr, thp->un.lcl.stacksize, 0,
								mempart_getid(thp->process, sys_memclass_id));
		...
		for(i = 0; i < prp->timers.nentries; ++i) {
			...
			if(VECP(tip, &prp->timers, i) && (tip->flags & _NTO_TI_ACTIVE) &&
					(prp->num_active_threads == 0 || tip->tid == thp->tid)) {
				timer_deactivate(tip);
			}
		}

		// Clean up after a SPORADIC thread 
		// At this point we have lost SS schedulalability, change to FIFO 
		...
		// queues be careful if another processor is already busy
		// processing interrupts
		{
			...
			do {
				...
				if(--loop == 0) crash();
#endif
				ilevel = get_inkernel() & INKERNEL_INTRMASK;
				intrevent_drain_check();
			} while(ilevel != 0);
		}

		// Purge any enqueued signals
		while((pup = thp->sig_pending)) {
			thp->sig_pending = pup->next;
	...
	if((thp = vector_rem(&prp->threads, thp->tid)) == NULL)
		crash();
	...
	if(prp->threads.nentries == prp->threads.nfree) {
		...
		while((pup = prp->sig_pending)) {
			prp->sig_pending = pup->next;
			object_free(prp, &pulse_souls, pup);
		}
...
if(thp != actives[KERNCPU]) crash();
					...
					thp->prev = (void *)guardsize;
					thp->next = (void *)lazystate;
			...
			guardsize = 0;
			if(memmgr.mmap(NULL, 0, thp->un.lcl.stacksize + guardsize, PROT_READ | PROT_WRITE,
					MAP_ANON|MAP_PRIVATE|MAP_STACK, 0, guardsize, __PAGESIZE, 0, NOFD,
					&thp->un.lcl.stackaddr, &thp->un.lcl.stacksize, mempart_getid(thp->process, sys_memclass_id)) != EOK) {
				thp->status = (void *)EAGAIN;
				thp->flags |= (_NTO_TF_KILLSELF | _NTO_TF_ONLYME);
				return;
			}
...
__SRCVERSION("nano_thread.c $Rev: 156323 $");
