...
ker_timer_create(THREAD *act, struct kerargs_timer_create *kap) {
	...
	// Most timers target the process, -1 indicates this
	tip->tid = (SIGEV_GET_TYPE(&tip->event) == SIGEV_SIGNAL_THREAD) ? 0 : -1;
...
ker_timer_settime(THREAD *act, struct kerargs_timer_settime *kap) {
	...
	if(kap->itime->nsec != 0) {
		...
		// If nessessary, set the thread id of to direct the signal
		if(tip->tid != -1) {
			tip->tid = act->tid;
		}
...
ker_timer_info(THREAD *act, struct kerargs_timer_info *kap) {
	...
	// Make sure timer is valid.
	id = kap->id;
	if((tip = vector_search(&prp->timers, id,
			(kap->flags & _NTO_TIMER_SEARCH) ? &id : NULL)) == NULL) {
		return EINVAL;
	}
...
__SRCVERSION("ker_timer.c $Rev: 153052 $");
