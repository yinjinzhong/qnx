	...
	if((cop->flags & COF_VCONNECT) == 0) {
		...
		if((chp = cop->channel)) {
			if(chp->flags & _NTO_CHF_DISCONNECT) {
				// Tell the server.
				_TRACE_COMM_EMIT_SPULSE_DIS(cop, cop->scoid|_NTO_SIDE_CHANNEL, prio|_PULSE_PRIO_HEAD);
				pulse_deliver(chp, prio|_PULSE_PRIO_HEAD,
...
__SRCVERSION("nano_connect.c $Rev: 153052 $");
