	...
	case NTO_DEBUG_GET_FPREG:			// pid:tid:debug_fpreg_t
		if(thp) {
			...
			if(fpudata) {
				...
				status = EOK;
			} else if(thp->un.lcl.tls->__fpuemu_data) {
...
static void kerext_debug_channel(void *args) {
	...
	for ( tp = chp->send_queue; tp != NULL; tp = tp->next ) {
		if ( tp->type == TYPE_PULSE ) {
			p->pulse_queue_depth += ((PULSE *)tp)->count;
		} else {
			p->send_queue_depth++;
		} 
	}
	for ( tp = chp->receive_queue; tp != NULL; tp = tp->next ) {
		p->receive_queue_depth++;
	}
	for ( tp = chp->reply_queue; tp != NULL; tp = tp->next ) {
		p->reply_queue_depth++;
	}
...
int DebugChannel(CHANNEL *chp, debug_channel_t *data) {
	...
	return __Ring0(kerext_debug_channel, &args);
}
...
__SRCVERSION("kerext_debug.c $Rev: 153052 $");
