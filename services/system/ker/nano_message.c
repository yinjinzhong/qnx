...
net_send2(KERARGS *kap, int vtid, CONNECT *cop, THREAD *thp) {
	...
	if(!net.chp) {
		...
		if(type == __KER_CHANNEL_DESTROY) {
			...
			LINK2_REM(thp);		// Remove vthread from reply queue
			thp->state = STATE_STOPPED;
			_TRACE_VTH_EMIT_STATE(thp, STOPPED);
		}
...
//			remove_unblock(thp, cop, kap->msg_replyv.rcvid);
		...
		++cop->links;		// Since Net's reply will decrement it
	}
	...
	block();
	act->blocked_on = cop;
	LINK2_BEG(net.chp->reply_queue, act, THREAD);
...
remove_unblock(THREAD *thp, CONNECT *cop, int rcvid) {
	...
	for(pup = (PULSE *)(void *)chp->send_queue; pup; pup = pup2) {
		pup2 = pup->next;
		if(pup->type == TYPE_PULSE  &&  pup->code == _PULSE_CODE_UNBLOCK  &&  pup->value == rcvid) {
			lock_kernel();
			LINK2_REM(pup);
...
__SRCVERSION("nano_message.c $Rev: 157350 $");
