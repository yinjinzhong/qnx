...
int trace_event(uint32_t* a_p)
{
	switch(_TRACE_GET_FLAG(*a_p)) {
		case _TRACE_GET_FLAG(_NTO_TRACE_INSERTSUSEREVENT):
		{
			(void) add_trace_event
			(
			 _TRACE_MAKE_CODE(RUNCPU, _TRACE_STRUCT_S, _TRACE_USER_C, (0x3ff)&*(a_p+1)),
			 NULL,
			 *(a_p+2),
			 *(a_p+3)
			);
		...
		case _TRACE_GET_FLAG(_NTO_TRACE_INSERTCUSEREVENT):
		{
			add_trace_buffer
			(
			 _TRACE_MAKE_CODE(RUNCPU, NULL, _TRACE_USER_C, (0x3ff)&*(a_p+1)),
			 (uint32_t*) *(a_p+2),
			 (uint32_t ) *(a_p+3)
			);
		...
		case _TRACE_GET_FLAG(_NTO_TRACE_INSERTSCLASSEVENT):
		{
			(void) add_trace_event
			(
			 _TRACE_MAKE_CODE(RUNCPU, _TRACE_STRUCT_S, (*(a_p+1))<<10, 
       (0x3ff)&*(a_p+2)),
			 NULL,
			 *(a_p+3),
			 *(a_p+4)
		...
		case _TRACE_GET_FLAG(_NTO_TRACE_INSERTCCLASSEVENT):
		{
			add_trace_buffer
			(
			 _TRACE_MAKE_CODE(RUNCPU, NULL, (*(a_p+1)<<10), (0x3ff)&*(a_p+2)),
			 (uint32_t*) *(a_p+3),
			 (uint32_t ) *(a_p+4)
			);
		...
		{
			trace_state_t t_s;
			uint32_t      	h=_TRACE_MAKE_CODE(RUNCPU, NULL, _TRACE_USER_C, (0x3ff)&*(a_p+1));
...
__SRCVERSION("nano_trace.c $Rev: 153052 $");
