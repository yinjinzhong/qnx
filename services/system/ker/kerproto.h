...
void                sync_mutex_unlock(THREAD *act, sync_t *sync);
...
unsigned	outside_vaddr_to_paddr_old(struct kdebug_entry *entry, uintptr_t *vaddr, unsigned *size);
...
unsigned    outside_get_inkernel(void);
int         outside_kdebug_path(struct kdebug_entry *, char *, int);
struct kdebug_entry *outside_kdebug_entry(unsigned sigcode, CPU_REGISTERS *context);
...
/* __SRCVERSION("kerproto.h $Rev: 153052 $"); */
