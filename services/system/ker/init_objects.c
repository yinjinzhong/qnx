...
init_objects() {
	...
	// Allocate fixed pool of interrupt events.
	grow_intrevents = num_intrevents - num_intrevents / 4;	// 75%
	(void)intrevent_alloc(num_intrevents, 1);
	mdriver_check();
	...
	/*
	 * before we go any further, associate procnto_prp with the system partition.
	 * This will allow all other allocations to be able to locate a partition
	 * of type sysram
	*/
	LIST_INIT(prp->mpart_list);
	MEMPART_ASSOCIATE(prp, mempart_getid(NULL, sys_memclass_id), mempart_flags_NONE);
...
int rdecl
intrevent_alloc(int num, int init) {
	int			 i;
	INTREVENT	*irp;

	for(i = 0 ; i < num ; ++i) {
		if((irp = _scalloc(sizeof(INTREVENT))) == NULL) {
			break;
		}

		// Want to keep interrupts disabled while we're initializing
		if (!init) {
			InterruptLock(&intrevent_lock);
		}

		LINK3_BEG(intrevent_free, irp, INTREVENT);

		if (!init) {
			InterruptUnlock(&intrevent_lock);
		}
	}
...
__SRCVERSION("init_objects.c $Rev: 156323 $");
