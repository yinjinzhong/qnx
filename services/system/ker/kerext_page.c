...
kerext_page_cont(void *data) {
	...
	if(kap->err != EOK) {
		...
		if(kap->code_signo & SIGCODE_KERNEL) {
			...
			usr_fault(kap->code_signo, thp, (uintptr_t)thp->next);
		}
	}
...
if(!(get_inkernel() & INKERNEL_LOCK)) crash();
#endif
#endif
	...
	if(sigevent_proc(&event)) {
		// Pulse failed, we must ready so the signal is delivered
		ready(thp);
		return -1;
	} 
...
kerext_page_wait_info(void *data) {
	...
	if(id < process_vector.nentries  &&
			...
			(thp->state == STATE_WAITPAGE)) {
		kap->vaddr = (uintptr_t)thp->next;
		*kap->ppid = prp->pid;
		*kap->ptid = thp->tid + 1;
		*kap->pflags = (unsigned)thp->status;

		pid = (unsigned)thp->prev;
		index = PINDEX(pid);
...
__SRCVERSION("kerext_page.c $Rev: 153052 $");
