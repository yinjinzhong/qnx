...
#define MUTEX_INIT(p, m) \
		do { \
			...
			(m)->mutex = tmp; \
			(m)->spin.value = 0; \
			sync_create(&(m)->mutex, 0); \
		} while(0)
#define MUTEX_DESTROY(p, m)	sync_destroy(NULL, &(m)->mutex)
...
kerext_process_create(void *data) {
	...
	LIST_INIT(prp->mpart_list);
	MUTEX_INIT(prp, &prp->mpartlist_lock);

	KASSERT((kap->extra != NULL) && (kap->extra->mpart_list != NULL));
	{
		int  i;
		mempart_list_t  *mpart_list = kap->extra->mpart_list;

		/* first thing is to associate with all specified partitions */
		for (i=0; i<mpart_list->num_entries; i++)
		{
			if ((status = MEMPART_ASSOCIATE(prp, mpart_list->i[i].t.id, mpart_list->i[i].flags)) != EOK)
			{
				MEMPART_DISASSOCIATE(prp, NULL);
...
kerext_process_destroy(void *data) {
	...
	// undoo all session stuff
	if(prp->session && --prp->session->links == 0) {
...
*/
	MEMPART_DISASSOCIATE(prp, NULL);
	MUTEX_DESTROY(prp, &prp->mpartlist_lock);
...
kerext_process_swap(void *data) {
	...
	if(parent->alarm) {
		parent->alarm->process = child;
		child->alarm = parent->alarm;
		parent->alarm = 0;
	}
...
ProcessShutdown(PROCESS *prp, int exec) {
	...
	if((r == -1) && (data.tlb_safe_cpu >= 0)) {
		...
		do {
			DBG_LOOP_CHK();
			/*
			 * The memory barriers will translate into bus synchronizing opcodes
...
__SRCVERSION("kerext_process.c $Rev: 156505 $");
