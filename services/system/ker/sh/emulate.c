	...
	if( ((flags & 0xff) == SIGBUS) && (((flags >> 8) & 0xff) == BUS_ADRALN) ) {
		...
		if(inst.i_d4.op == OPCODE_MOVW_R0_ADRN) {
			...
			*addr = val16.bytes.b1;
			...
			/* mov.w @(disp,Rm),R0 */
			// sign extended
			addr = (int8_t*)(ctx->gr[inst.i_d4.reg] + (inst.i_d4.disp << 1));
			val16.bytes.b0 = *addr++;
			val16.bytes.b1 = *addr;
			ret = 0;
			lock_kernel();
			...
			*addr = val32.bytes.b3;
			...
			if(inst.i_r.func == FCODE_MOVW_RM_AR0RN) {
				...
				*addr = val32.bytes.b3;
				...
				/* mov.w @(R0,Rm),Rn */
				// sign extended
				addr = (int8_t*)(ctx->gr[inst.i_r.reg2] + ctx->gr[0]);
				val16.bytes.b0 = *addr++;
				val16.bytes.b1 = *addr;
				ret = 0;
				lock_kernel();
			...
			if(inst.i_r.func == FCODE_MOVW_ARM_RN) {
				/* mov.w @Rm,Rn */
				// sign extended
				addr = (int8_t*)ctx->gr[inst.i_r.reg2];
				val16.bytes.b0 = *addr++;
				val16.bytes.b1 = *addr;
				ret = 0;
				lock_kernel();
...
__SRCVERSION("emulate.c $Rev: 153052 $");
