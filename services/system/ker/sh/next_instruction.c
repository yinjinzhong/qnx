...
void	tlb_flush_va(unsigned asid, uintptr_t start, uintptr_t end);
...
next_instruction(CPU_REGISTERS *ctx) {
	...
	if(memmgr.vaddrinfo(aspaces_prp[KERNCPU], ctx->pc, &phys, NULL, VI_PGTBL) == PROT_NONE) return -1;
	if ( SH_PHYS_IN_1TO1(phys) ) {
		...
		InterruptDisable();
		asid = SH_CCN_ASID_FROM_PTEH(in32(SH_MMR_CCN_PTEH));
		
		// Ensure there aren't any mappings for vaddr already in the TLBs
		tlb_flush_va( asid, vaddr, vaddr+2 );
