...
# FIXME_FPU: issues related to FPU handling
...
1:
	...
	or		r3, r0					! set cpunum
	.word	0x0173					! mov.co r0, @r1
	bf		0b						! mov.co failed - retry whole attempt
...
1:	.word	0x0163					! mov.li @r1, r0
	mov		r0, r13					! stash old inkernel value
	tst		#INKERNEL_NOW, r0
	bt		2f						! INKERNEL_NOW is clear
	and		r2, r0
	cmp/eq	r3, r0
	bf		1b						! we don't own the kernel
...
bad_func_emulation:
	...
	# FIXME_HAO: is this correct?
	# SH_KER_TRAP_BOUNDARY is 0xfd
	# trapa #0xff -> cmpxchg emulation
	# trapa #0xfd -> __inline_DebugKDBreak
	# trapa #0xfc -> __inline_DebugKDOutput
...
.endif
	...
	#
	# act->flags &= ~(_NTO_TF_KERERR_SET|_NTO_TF_KERERR_LOCK|_NTO_TF_BUFF_MSG)
	#
	mov.l	Lke_TFLAGS, r0
...
Lke_TF_mask:				.long	~(_NTO_TF_KERERR_SET | _NTO_TF_BUFF_MSG | _NTO_TF_KERERR_LOCK)
...
2:
	...
	#
.ifdef	VARIANT_smp
0:	.word	0x0763						! mov.li @r7, r0
	add		#1, r0
	.word	0x0773						! mov.co r0, @r7
	bf		0b
.else
	add		#1, r7
.endif
	...
	# FIXME_FPU: probably only want to do this on entry from user mode?
	#
...
case2:
	...
	mov.b	@r2, r2							! queued_event_priority
	mov.b	@(r0, r12), r0					! actives[RUNCPU]->priority
	#
	# Note that even though mov.b above sign-extends, this test works because
	# we are doing an unsigned comparison
	#
	cmp/hi	r0, r2
	bf		0f								! goto __ker_exit2
...
__exc_general_exptable:
	.long	__exc_fpu_disable		/* 0x800 */
	.long	__exc_fpu_disable_slot	/* 0x820 */
...
__exc_fpu_disable_slot:
	...
	#
	# Save current active FPU context
	#
...
0:
	...
	#
	# If we entered from kernel/interrupt, simply enable the FPU
	# We saved the active context above, and since we disable the FPU in
	# __ker_exit, a subsequent user FPU access will fault allowing us to
	# restore the correct context.
	#
	tst		r14, r14
	bt		__exc_fpu_restore

	#
	# Set correct fpscr bits
	#
...
Lef_FPSCR_DN_PR:		.long	SH_FPSCR_DN | SH_FPSCR_PR
Lef_FPSCR_DN_PR_FR:		.long	SH_FPSCR_DN | SH_FPSCR_PR | SH_FPSCR_FR
.ifdef	VARIANT_smp
...
Lfs_SH_FPU_REG_SIZE:	.long	SH_FPU_REG_SIZE
Lfs_FPSCR_DN_PR_FR:		.long	SH_FPSCR_DN | SH_FPSCR_PR | SH_FPSCR_FR
Lfs_FPSCR_DN_PR:		.long	SH_FPSCR_DN | SH_FPSCR_PR
