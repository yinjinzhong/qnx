...
interrupt_detach_entry(PROCESS *prp, int index) {
	...
	LINK1_REM(interrupt_level[itp->level].queue, itp, INTERRUPT);
       ...
       // code since we're about to destroy the INTERRUPT object
       do { } while(get_inkernel() & INKERNEL_INTRMASK);
#endif
...
interrupt_unmask(int level, INTERRUPT *itp) {
	...
	if(alives[0]) CPU_SLOCK_INTR_LOCK(&intr_slock);
...
__SRCVERSION("nano_interrupt.c $Rev: 153052 $");
