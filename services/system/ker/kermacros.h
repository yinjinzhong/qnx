...
//
	// This pulse should be time-ordered at the head (all bits on in field)
#define _PULSE_COUNT_HEAD			((2<<(sizeof(((struct pulse_entry *)0)->count)*8-1)) - 1)
	// Maximum count for compressing pulses
#define _PULSE_COUNT_MAX			(_PULSE_COUNT_HEAD-1)
...
#define MUTEXHOLDLIST_PRI(_queue, _object) {\
	SYNC **_prev;\
	for(_prev = &(_queue);\
		*_prev && (*_prev)->waiting->priority >= (_object)->waiting->priority;\
		_prev = &((*_prev)->waiting->args.mu.next));\
	if(((_object)->waiting->args.mu.next = (*_prev)))\
		(*_prev)->waiting->args.mu.prev = &(_object)->waiting->args.mu.next;\
	(_object)->waiting->args.mu.prev = _prev;\
	*_prev = (_object);\
	}

#define MUTEXHOLDLIST_REM(_object) {\
	if((*((_object)->waiting->args.mu.prev) = (_object)->waiting->args.mu.next))\
		(_object)->waiting->args.mu.next->waiting->args.mu.prev = (_object)->waiting->args.mu.prev;\
	}
...
#define _PULSE_PRIO_HEAD        0x00004000
...
//flag options for signal_kill_process and signal_kill_thread 
...
{ \
	...
	} while((tsp) != qtimeptr->nsec); \
	if((incl_tod)) { \
		int64_t		adjust; \
		do { \
			adjust = qtimeptr->nsec_tod_adjust; \
			/* Try again if the time has changed under our feet. */ \
		} while(adjust != qtimeptr->nsec_tod_adjust); \
		(tsp) += adjust; \
	} \
}
...
/* __SRCVERSION("kermacros.h $Rev: 157350 $"); */
