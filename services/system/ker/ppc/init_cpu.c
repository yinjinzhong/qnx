...
static int (*real_vmm_fault)(struct fault_info *info);
static unsigned
...
static void
install_fault_shim() {
	real_vmm_fault = memmgr.fault;
	memmgr.fault = (int (*)(struct fault_info *))vmm_fault_shim;
}
...
__SRCVERSION("init_cpu.c $Rev: 154627 $");
