...
#		immediately. Rather, we only set it once it is
#
#
...
.endif
	...
	lwz		%r6,TFLAGS(%r31)
	loadi	%r4,(_NTO_TF_KERERR_SET | _NTO_TF_BUFF_MSG | _NTO_TF_KERERR_LOCK)
	slwi	%r3,%r0,2
	andc	%r5,%r6,%r4
	stw		%r0,SYSCALL(%r31)
...
.ifdef VARIANT_smp
	...
	lis		%r3,MEMMGR_FAULT+memmgr@ha
	lwz		%r3,MEMMGR_FAULT+memmgr@l(%r3)
	mr		%r4,%r27
	mtctr	%r3
...
case2:
	...
	lbz		%r0,PRIORITY(%r31)
	lbz		%r3,queued_event_priority@sdarel(%r13)
	cmpw	%r0,%r3
	bge		__ker_exit2	
...
from_kerorintr:
.ifdef VARIANT_smp
	# %r3 has the MSR value from above
 	IS_CPUININTR	%r3, %r7
...
__exc_instr_access:
#	mfspr	%r4,SRR0				# get address responsible
#	MMU safe
...
3: 	
	...
	mfmsr	%r16
	ori		%r7,%r16,0
	SET_CPUININTR %r7			# Mark as in interrupt 
	or		%r7,%r7,%r24		# Interrupts on
	mtmsr	%r7
.else	
	#	
...
1:
	...
	mtmsr	%r16					# IRQ's off, inintr bit off
	mr		%r4,%r28
	cmpw	%r15,%r30
	beq+	usr_access_exc
