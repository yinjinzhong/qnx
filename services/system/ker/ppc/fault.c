...
static void
memcpy_w(uint8_t *dst, uint8_t *src) {
	*(dst + 3) = *(src + 3);
	*(dst + 2) = *(src + 2); 
	*(dst + 1) = *(src + 1);
	*dst       = *src;
...
static void
memcpy_wr(uint8_t *dst, uint8_t *src) {
	*(dst + 3) = *src;
	*(dst + 2) = *(src + 1); 
	*(dst + 1) = *(src + 2);
	*dst       = *(src + 3);
...
static void
loadh(uint8_t *target, uint8_t *source, uint8_t f_sign) {
...
static void
loadh_r(uint8_t *target, uint8_t *source) {
#if defined(__BIGENDIAN__)
	*target = 0;
	*(target + 1) = 0;
	*(target + 3) = *source;
...
static void
storeh(uint8_t *target, uint8_t *source) {
#if defined(__BIGENDIAN__)
	*target  = *(source+2);
	*(target + 1) = *(source+3);
#elif defined(__LITTLEENDIAN__)
...
static void
storeh_r(uint8_t *target, uint8_t *source) {
#if defined(__BIGENDIAN__)
	*target  = *(source+3);
	*(target + 1) = *(source+2);
#elif defined(__LITTLEENDIAN__)
...
__SRCVERSION("fault.c $Rev: 153052 $");
