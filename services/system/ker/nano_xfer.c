 ...
 void xfer_restorestate(THREAD *act) {
	...
	if(act->internal_flags & _NTO_ITF_MSG_DELIVERY) {
		...
		if((thp = act->blocked_on)) {
			act->blocked_on = 0;
#ifndef NDEBUG
		if((thp->type != TYPE_THREAD) && (thp->type != TYPE_VTHREAD)) {
...
int xfermsg(THREAD *dthp, THREAD *sthp, int doff, int soff) {
	...
	if(dthp->restart != sthp || sthp->restart != sthp || sthp->args.ms.dthp != dthp) {
		...
		sthp->restart = dthp->restart = sthp;
	} else {
		...
		if(sprp == prp) {
			if(dprp != prp) {
				...
				do {
					...
					soff += nbytes;
					while(soff >= (nbytes = GETIOVLEN(src))) {
						soff -= nbytes;
			...
			do {
				...
				doff += nbytes;
				while(doff >= (nbytes = GETIOVLEN(dst))) {
					doff -= nbytes;
...
__SRCVERSION("nano_xfer.c $Rev: 153052 $");
