...
clock_handler(void *dummy, int id) {
	...
	*/
	...
	if(qtp->adjust.tick_count  &&  --qtp->adjust.tick_count == 0) {
...
snap_time(uint64_t *tsp, int incl_tod) {
	...
	} while(*tsp != qtp->nsec || adjust != qtp->nsec_tod_adjust);
	if(incl_tod) {
		*tsp += adjust;
	}
}
...
kerext_clock_attach(void *data) {
	...
	lock_kernel();
	id = interrupt_attach(level, &clock_handler, 0, 0);
	if(id >= 0) {
...
void
clock_attach(void) {
	if(__Ring0(kerext_clock_attach, NULL) < 0) {
		kprintf("clock intr failure (%d)\n", errno);
		crash();
	}
}

__SRCVERSION("nano_clock.c $Rev: 155349 $");
