...
static void rdecl debug_sigset(PULSE *pup, sigset_t *ssp) {
	for(SIGMASK_ZERO(ssp); pup; pup = pup->next) {
...
int rdecl debug_thread(PROCESS *prp, THREAD *thp, debug_thread_t *dtp) {
	...
	case STATE_WAITPAGE: {
		...
		dtp->blocked.waitpage.pid = 0;
		if((id = (unsigned)thp->prev & 0xffff) > 0 && (prp = vector_lookup(&process_vector, id))) {
			dtp->blocked.waitpage.pid = prp->pid;
		}
		dtp->blocked.waitpage.vaddr = (intptr_t)thp->next;
		dtp->blocked.waitpage.flags = (unsigned)thp->prev >> 16;
		break;
	}
	case STATE_MUTEX:
	case STATE_CONDVAR:
...
int rdecl debug_run(PROCESS *prp, debug_run_t *drp) {
	...
	if((drp_flags & _DEBUG_RUN_CLRSIG) && dep->why == _DEBUG_WHY_SIGNALLED) {
		(void)signal_clear_thread(thp, &prp->siginfo);
		dep->why = _DEBUG_WHY_REQUESTED;
	}
...
__SRCVERSION("nano_debug.c $Rev: 153052 $");
