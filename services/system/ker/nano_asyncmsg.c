...
msgsend_gbl(THREAD *act, CONNECT *cop, void *msg, size_t size, unsigned priority, int coid) {
	...
	if(chp->num_curr_msg >= chp->max_num_buffer) {
		...
		act->blocked_on = cop;
		LINK2_PRI(chp->ch.send_queue, act, THREAD);
		++cop->links;
	} else {
...
if(gblmsg == NULL) crash();
		...
		act->args.ri.id = coid;
		LINK2_BEG(chp->ch.receive_queue, act, THREAD);
	}
...
__SRCVERSION("nano_asyncmsg.c $Rev: 157350 $");
