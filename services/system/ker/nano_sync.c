...
SYNC * rdecl
sync_create(sync_t *sync, unsigned flags) {
	SYNC			*syp;
	OBJECT			*obp;
	unsigned		addr;
...
int rdecl
sync_destroy(PROCESS *prp/*not currently used*/, sync_t *sync) {
	OBJECT		*obp;
	unsigned	addr;
	SYNC		*syp;
	THREAD		*thp;
...
sync_lookup(sync_t *sync, unsigned create) {
	...
	lock_kernel();
	if((syp = sync_create(sync, PTHREAD_PROCESS_PRIVATE)) == NULL) {
		return(NULL);
	}
			...
			if(syp->obj == obp  &&  syp->addr >= addr1  &&  syp->addr <= addr2) {
				...
				if(prp) {
					...
					if(SYNC_PINDEX(sync->__owner) == SYNC_PINDEX(SYNC_OWNER_BITS(prp->pid, 0))) {
						...
						if((thp = syp->waiting) && ((thp->type == TYPE_THREAD && thp->state == STATE_MUTEX) || (thp->type == TYPE_SYNCEVENT && ((struct syncevent_entry *)thp)->subtype == SYNCEVENT_SUBTYPE_PRIORITYCEILING))) {
					...
					for(syncep = (struct syncevent_entry*) syp->waiting; syncep;) {
						if(syncep->type == TYPE_SYNCEVENT && syncep->subtype == SYNCEVENT_SUBTYPE_EVENT) {
							prp1 = syncep->un.ev.process;
							if(prp1 == prp) {
								if(prp->guardian == NULL) {
									struct syncevent_entry *tmp;
					...
					if(event_state == EVENT_DELIVER) {
						...
						while((thp = syp->waiting)) {
							if(thp->type != TYPE_THREAD) {
								if(!((thp = thp->next) && thp->type == TYPE_THREAD)) {
									break;
								}
							}
					...
					if((thp = syp->waiting)) {
						if(((thp->type == TYPE_THREAD && thp->state == STATE_MUTEX) || (thp->type == TYPE_SYNCEVENT && ((struct syncevent_entry *)thp)->subtype == SYNCEVENT_SUBTYPE_PRIORITYCEILING))
						  && thp->args.mu.owner) {
							MUTEXHOLDLIST_REM(syp);
							thp->args.mu.owner = 0;
						}
...
__SRCVERSION("nano_sync.c $Rev: 157318 $");
