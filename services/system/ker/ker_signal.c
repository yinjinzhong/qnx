...
do_ker_signal_kill(THREAD *act, struct kerargs_signal_kill *kap) {
	...
	if(kap->pid == 0) kap->pid = -act->process->pgrp;

	// Verify the signal and code are valid
	if(kap->signo < 0  ||  kap->signo > _SIGMAX  || kap->code > SI_USER) {
		return EINVAL;
	}
	...
	if (kap->pid == -1) {
		...
		if(!keriskill(act, prp, kap->signo)) {
			return ENOERROR;
		}
		...
		pid = act->process->pid;
		if(kap->tid == 0) {
			status = signal_kill_process(prp, kap->signo, kap->code, kap->value, pid, 0);
		} else {
			status = signal_kill_thread(prp, thp, kap->signo, kap->code, kap->value, pid,0 );
		}
	}
	else {
		lock_kernel();
		...
		pid = act->process->pid;
		status = signal_kill_group(act, -kap->pid, kap->signo, kap->code, kap->value, pid);
	}
...
ker_signal_procmask(THREAD *act, struct kerargs_signal_procmask *kap) {
	...
	if(kap->how == SIG_PENDING) {
		...
		for(pup = thp->sig_pending; pup ; pup = pup->next) {
			...
			if (pup->type == TYPE_SIGNAL) { // only TYPE_SIGNAL
				n = PRI_TO_SIG(pup->priority);
				SIG_SET(newbits, n);
			}
		}
		for(pup = thp->process->sig_pending; pup ; pup = pup->next) {
			unsigned	n;
...
ker_signal_waitinfo(THREAD *act, struct kerargs_signal_wait *kap) {
	PULSE			*pup;
	siginfo_t		*sip = kap->sig_info;
	sigset_t		 sig_wait = *kap->sig_wait;
	int				 signo, i;
	unsigned		 tls_flags;

	RD_VERIFY_PTR(act, kap->sig_wait, sizeof(*kap->sig_wait));
	if(kap->sig_info!=NULL)	 {
		WR_VERIFY_PTR(act, kap->sig_info, sizeof(*kap->sig_info));
	}
	// Check for a pending signal.
	// We first check the thread than the process.
	for(i = 2, pup = act->sig_pending ; i-- ;) {
		for(; pup ; pup = pup->next) {
			...
			if(SIG_TST(sig_wait, signo)) {
				...
				// Signals may be queued. Release signal object on last one.
				if(pup->count == _PULSE_COUNT_HEAD || --pup->count == 0) {
					LINK2_REM(pup);
					object_free(act->process, &pulse_souls, pup);
				}
...
__SRCVERSION("ker_signal.c $Rev: 154836 $");
