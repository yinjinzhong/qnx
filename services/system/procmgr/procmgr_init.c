...
static int proc_start(PROCESS *prp, uintptr_t *start_ip) {
	...
	if((lcp->state & LC_STATE_MASK) == LC_FORK) {
		...
		if(!(lcp->flags & _FORK_ASPACE)) {
			...
			size = sp - lcp->start.stackaddr;
#else
			size = lcp->start.stackaddr - sp;
#endif
			if(size >= sizeof buff) {
				return ENOMEM;
			}
		...
		thp->flags |= (_NTO_TF_KERERR_SET | _NTO_TF_KERERR_LOCK); 
		return EOK;
	}
...
void procmgr_init(void) {
	...
	proc_core_code = pulse_attach(dpp, MSG_FLAG_ALLOC_PULSE, 0, procmgr_coredump, NULL);
  	ConnectServerInfo(SYSMGR_PID, PROCMGR_COID, &info);
...
__SRCVERSION("procmgr_init.c $Rev: 153052 $");
