...
cpu_pte_manipulate(struct mm_pte_manipulate *data) {
	...
	if(data->shmem_flags & SHMCTL_LAZYWRITE) {
		...
		} else if(PTE_OP_UNMAP) {
			paddr_t		paddr;

			// The unmap doesn't set the paddr, so we have to get it from
			// the page table.
			if(cpu_vmm_vaddrinfo(object_from_data(adp, address_cookie), curr, &paddr, NULL) != PROT_NONE) {
...
__SRCVERSION("cpu_vmm.c $Rev: 156323 $");
