...
vmm_mapinfo(PROCESS *prp, uintptr_t vaddr, struct _procfs_map_info *mip, 
	...
	if(or != NULL) {
		...
		if((mm->mmap_flags & MAP_ELF) && (mm->mmap_flags & PROT_WRITE)) {
			...
			map_isolate(&pms, &adp->map, mm->start - 1, 0, MI_NEXT);
			prev_mm = pms.first;
			if(prev_mm != NULL 
			  && ((prev_mm->mmap_flags & (PROT_READ|PROT_WRITE|MAP_ELF)) == (PROT_READ|MAP_ELF))
			  && (prev_mm->obj_ref != NULL)) {
				name_mm = prev_mm;
			}
			map_coalese(&pms);
		}
		...
		if(mdp != NULL) {
			...
			if((nobp->hdr.type == OBJECT_MEM_SHARED) && (nobp->mem.mm.flags & MM_SHMEM_IFS)) {
				...
				} else if(nobp->hdr.type == OBJECT_MEM_ANON) {
					memcpy(mdp->path, "/dev/zero", sizeof("/dev/zero"));
				//RUSH2: Do something with OBJECT_MEM_TYPED here, or in tymem_name?
				} else {
					memcpy(mdp->path, "/dev/mem", sizeof("/dev/mem"));
				}
			}
		}
...
__SRCVERSION("vmm_mapinfo.c $Rev: 153052 $");
