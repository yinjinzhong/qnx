...
#include "vmm.h"
	...
	#error this file does not work with SMP - see the MIPS version for what needs to be added
#endif
...
wire_sync(ADDRESS *adp) {
	...
	we = adp->cpu.wires;
	InterruptDisable();
	if(adp->cpu.pending_wire_delete) {
		ppcbke_tlb_t	dead;

		adp->cpu.pending_wire_delete = 0;
...
wire_check(struct mm_pte_manipulate *data) {
	...
	we = adp->cpu.wires;
	vaddr = data->first;
	pt = &adp->cpu.pgdir[L1PAGEIDX(vaddr)][L2PAGEIDX(vaddr)];
	new.ts = 1;
	new.epn = VA_INVALID;
	new.rpn = PTE_PADDR(pt);
	new.attr = pt->flags & 0x1f;
...
need_sync:	
	if(adp->cpu.asid == get_spr(PPCBKE_SPR_PID)) {
		wire_sync(adp);
	}
}
