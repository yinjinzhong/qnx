...
alloc_hashtable() {
	...
	mempart_id_t		mpid = mempart_getid(NULL, sys_memclass_id);
...
set_one_bat(ADDRESS *adp, unsigned bi) {
	...
	if(up != ~0) {
		...
		set_spr_indirect(PPC_SPR_IBAT0U + adj, up);
	}
}
...
#define GET_ACTIVE_ASID()	((get_sreg(0x40000000) & 0x00ffffff) - 0x400000)
