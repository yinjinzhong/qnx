...
EXTERN struct pa_restrict	*restrict_user_4G_first;
...
EXTERN uintptr_t			system_heap_vaddr_lo;
EXTERN uintptr_t			system_heap_vaddr_hi;
...
struct pa_quantum	*pa_alloc(paddr_t size, paddr_t align, unsigned colour,
						...
						struct pa_restrict *restrict, paddr_t resv_size);
...
void				pa_free_info(struct pa_restrict *restrict, paddr_t *total, paddr_t *contig);
int					pa_sysram_overlap(struct pa_restrict *restrict);
...
extern void					fdmem_done_prep(OBJECT *obp);
...
#define MM_MEM_INPROGRESS			0x40000000
#define MM_MEM_SKIPLOCKCHECK		0x20000000
#define MM_MEM_VISIBLE				0x10000000
...
#define MM_MEM_COLOUR_FLUSH			0x02000000
	 ...
	 */
// These next two are temp kludges until I can figure out why
// my proc_mux_lock changes didn't work.
#define MM_MEM_HOLD_MASK			0x00ff0000
#define MM_MEM_HOLD_INC				0x00010000
...
#define MM_FDMEM_ZOMBIE				0x00000002
#define MM_FDMEM_CLEANUP			0x00000004
...
#define VERIFY_OBJ_LOCK(o)	CRASHCHECK(!proc_mux_haslock(&(o)->mem.mm.mux))


/* __SRCVERSION("mm_internal.h $Rev: 157184 $"); */
