...
fault_pulse(message_context_t *ctp, int code, unsigned _flags, void *handle) {
	...
	have_lock = proc_lock_owner_check(prp, pid, tid);
	if(!have_lock) {
	...
	r = memory_reference(&mm, vaddr, vend, (sigcode & SIGCODE_STORE) ? MR_WRITE : MR_NONE, &ms);
	lcp = prp->lcp;
	if((lcp != NULL) && (r != EOK)) {
		lcp->fault_errno = r;
	}
...
//fail3:
	ProcessBind(0);
	if(!have_lock) proc_unlock_adp(prp);
...
__SRCVERSION("vmm_fault.c $Rev: 157123 $");
