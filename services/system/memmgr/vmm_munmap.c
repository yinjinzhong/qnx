	...
	if(pq->blk == PAQ_BLK_FAKE) {
		...
		do {
			...
			if(pq_flags & PAQ_FLAG_HAS_SYNC) {
				...
				// Make sure the page tables are in place so we don't
				// take a fault (we'd deadlock trying to aquire the
				// aspace lock in fault_pulse()). This opens up a small
				// window since we're turning on write permission
				// unconditionally (so kernel can set the mutex state to
				// dead if it needs to). We're going to unmap the region
				// very shortly so it should be ok for right now.
				// What would be better is if we could figure out a
				// way for the fault_pulse() to realize that the aspace
				// and object are already locked and it doesn't have
				// to do the locking itself - note that you'd still have
				// to deal with an owned mutex in a read-only region 
				// somehow.
				// RUSH3: proc_lock_owner_mark() will work to let fault_pulse()
				// RUSH3: know that the addr space is locked, but still have to 
				// RUSH3: think about object. Also, what about read only regions?
				(void)pte_map(prp->memory, 
						ROUNDDOWN(vaddr, QUANTUM_SIZE), vaddr|(QUANTUM_SIZE-1),
						PROT_READ|PROT_WRITE|mm->mmap_flags,
						obp, paddr, 0);
				sync_off = PADDR_TO_SYNC_OFF(paddr);
				MemobjDestroyed(PADDR_TO_SYNC_OBJ(paddr),
...
ms_unmap(ADDRESS *adp, struct map_set *ms, int unmap_flags) {
	...
	//RUSH3: Indicate we hold the aspace locked so that we don't deadlock
	//RUSH3: in fault_pulse() if we fault during the clean_pmem().
	//RUSH3: This will only prevent deadlocks for write faults on ~PROT_WRITE
	//RUSH3: mappings - read faults will still deadlock trying acquire the
	//RUSH3: object lock that we hold here.
	...
	for(mm = ms->first; mm != end_mm; mm = mm->next) {
		...
		if(!(data.unmap_flags & UNMAP_NORLIMIT)) {
			...
			if((mm->mmap_flags & (MAP_ANON|MAP_STACK|MAP_TYPE)) == (MAP_ANON|MAP_PRIVATE)) {
		...
		if(or != NULL) {
			...
			if(obp->hdr.type != OBJECT_MEM_ANON) {
				if(((mm->mmap_flags & (MAP_LAZY|MAP_TYPE)) == MAP_PRIVATE) 
					...
					memobj_pmem_del(anmem_obp, anmem_off, anmem_size);
					if(!anmem_lock) memobj_unlock(anmem_obp);
				}
			}
	
			if((data.flags & PAQ_FLAG_INUSE) && (data.start < data.end)) {
				memobj_pmem_del(obp, data.start, (size_t)(data.end - data.start) + 1);
			}
...
int 
vmm_munmap(PROCESS *prp, uintptr_t vaddr, size_t len, int flags, mempart_id_t mpart_id) {
	int				r;
	struct map_set	ms;
	ADDRESS			*adp;

	if(prp == NULL) {
		KASSERT(mpart_id != mempart_id_t_INVALID);
