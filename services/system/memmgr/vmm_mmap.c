...
int
tymem_pmem_alloc(OBJECT *obp, struct map_set *ms, struct mm_map *mm, 
	...
	if(flags & (IMAP_TYMEM_ALLOCATE|IMAP_TYMEM_ALLOCATE_CONTIG)) {
		...
		if(obp->mem.mm.flags & MM_MEM_RDB) {
			...
			if (MEMPART_CHK_and_INCR(obp->hdr.mpid, size, &resv) != EOK)
				return ENOMEM;
			pq = pa_alloc(size, __PAGESIZE, PAQ_COLOUR_NONE, pa_flags, NULL,
						obp->mem.mm.restrict, resv);
			if (pq == NULL)
				MEMPART_UNDO_INCR(obp->hdr.mpid, size, resv);
		}
		if(pq == NULL) return ENOMEM;
	} else if(flags & IMAP_TYMEM_MAP_ALLOCATABLE) {
		r = tymem_valid_offset(boff, size, obp->mem.mm.restrict);
		if(r != EOK) return r;
		data.pq = NULL;
		data.owner = &data.pq;
	...
	for( ;; ) {
		...
		while(pq_add != NULL) {
			...
			if(mm->reloc) {
				mm->reloc = 0;
				memobj_pmem_del(obp, add_off, NQUANTUM_TO_LEN(pq_add->run));
			}
...
vmm_mmap(PROCESS *prp, uintptr_t vaddr_requested, size_t size_requested, 
		...
		unsigned preload, int fd, void **vaddrp, size_t *sizep, mempart_id_t mpart_id) {
	uintptr_t			vaddr;
	size_t				size;
	size_t				guardsize = 0;
	size_t				pg_offset;
	...
	if(prp == NULL) {
		 ...
		 * mempart_id_t_INVALID and sys_mempart and procnto_prp both
		 * be NULL. We allow this to go through otherwise we assert
		 * mpart_id != mempart_id_t_INVALID
		*/
		if (mpart_id == mempart_id_t_INVALID) {
		...
		if(flags & MAP_ANON) {
			...
			if (MEMPART_CHK_and_INCR(mpart_id, size_requested, &resv) != EOK)
				return ENOMEM;
			pq = pa_alloc(size_requested, size_requested, PAQ_COLOUR_NONE, 
		...
		*sizep = size_requested;
		if(CPU_1TO1_IS_PADDR(boff)) {
			...
			vaddr = data.start;
			if((vaddr == VA_INVALID) && (flags & MAP_ANON)) {
		...
		*vaddrp = (uint8_t *)vaddr + pg_offset;
		if(pq != NULL) {
			if(vaddr < system_heap_vaddr_lo) system_heap_vaddr_lo = vaddr;
			vaddr += size_requested - 1;
			if(vaddr > system_heap_vaddr_hi) system_heap_vaddr_hi = vaddr;
		}
		return EOK;
	}
	...
	if((flags & (MAP_ANON|MAP_STACK|MAP_TYPE)) == (MAP_ANON|MAP_PRIVATE)) {
	...
	if((obp != NULL) 
		...
		case OBJECT_MEM_SHARED:
			end_page = boff;
			end_vaddr = vaddr_requested + size_requested;
			vaddr = ROUNDDOWN(end_vaddr, __PAGESIZE);
			if(vaddr >= vaddr_requested) {
				end_page += vaddr - vaddr_requested;
			}
			if(end_page >= obp->mem.mm.size) {
	...
	if((mask == 0) && (vaddr_requested == 0)) {
		...
		// aligned in such a manner that we can use big pages.
		mask = 1;
	}
	...
	if(obp->hdr.type == OBJECT_MEM_TYPED) {
		r = tymem_pmem_alloc(obp, &ms, mm, flags, boff, size);
		if(r != EOK) goto fail3;
		if(flags & (IMAP_TYMEM_ALLOCATE|IMAP_TYMEM_ALLOCATE_CONTIG)) {
			if(!(obp->mem.mm.flags & MM_MEM_RDB)) {
				flags |= MAP_SYSRAM;
		...
		if(!(flags & MAP_LAZY)) {
			...
			end = min(boff+size, obp->fdmem.mem.mm.size);
			// Make sure we have physical memory to cover the mapping...
			r = memobj_pmem_add(obp, boff, end - boff, flags);
			if(r != EOK) goto fail3;
		}
		flags |= MAP_SYSRAM;
	} else if(flags & MAP_ANON) {
	...
	if(preload == 0) {
		...
		if(flags & MAP_STACK) {
			...
			case OBJECT_MEM_SHARED:	
				...
				if((flags & PROT_WRITE|MAP_TYPE) == (PROT_WRITE|MAP_SHARED)) {
					preload = 8*__PAGESIZE;
				}
				break;
			default: 
...
fail5:
	if(anmem_off != ~(off64_t)0) {
		memobj_lock(anmem_obp);
		memobj_pmem_del(anmem_obp, anmem_off, size);
		memobj_unlock(anmem_obp);
	}
...
fail4:
	if(obp->hdr.type == OBJECT_MEM_TYPED) {
		...
		for(mm = ms.first; mm != ms.last->next; mm = mm->next) {
			memobj_pmem_del(obp, mm->offset, ( mm->end - mm->start ) + 1);
		}
	} else if(flags & MAP_ANON) {
		memobj_pmem_del(obp, boff, size);
	}
...
__SRCVERSION("vmm_mmap.c $Rev: 157184 $");
