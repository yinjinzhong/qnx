	...
	for(i = 0; i < NUM_PROCESSORS; ++i) {
		...
		if(VALID_ASID(asid)) {
			if(i == RUNCPU) {
				...
				if(icache_flags & CACHE_FLAG_VIRTUAL) {
...
#if CPU_SYSTEM_HAVE_MULTIPLE_PAGESIZES
int
...
//kprintf("splitting %x from size %x\n", check, pgsz);
					for( ;; ) {
						...
						//temporarily unmap the entries first so that we
...
__SRCVERSION("cpu_vmm.c $Rev: 157044 $");
