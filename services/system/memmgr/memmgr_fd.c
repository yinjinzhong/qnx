...
void
fdmem_done_prep(OBJECT *obp) {
	// We keep a pending count of how many people are about to call
	// object_done() on this object - happens before we unlock the object.
	++obp->fdmem.pending_dones;
}
...
fdmem_done(OBJECT *obp) {
	...
	r = 0;
	if(!(obp->mem.mm.flags & MM_FDMEM_CLEANUP)) {
		pthread_mutex_lock(&fdmem_mutex);
		// We only try to free the object if there aren't any other pending
		// dones.
		CRASHCHECK(obp->fdmem.pending_dones == 0);
		if(--obp->fdmem.pending_dones != 0) {
			pthread_mutex_unlock(&fdmem_mutex);
			return 0;
		}
	}
	flags = obp->mem.mm.flags;

	// We might get recursively invoked, so make sure we're not 
	// already working on this object.
	if(!(flags & MM_MEM_INPROGRESS)) {
		obp->mem.mm.flags |= MM_MEM_INPROGRESS;
		if(flags & MM_FDMEM_CLEANUP) {
			...
			r = 1;
		} else if((obp->mem.mm.refs == NULL) && (obp->mem.mm.mux == NULL)) {
			obp->mem.pm.mtime = time(0);
			if(flags & MM_FDMEM_STICKY) {
				obp->mem.pm.mtime += STICKY_TIMEOUT;
			}
			if(!(flags & MM_FDMEM_ZOMBIE)) {
				// Put it on the zombie list
				obp->mem.mm.flags |= MM_FDMEM_ZOMBIE;
			...
			if(flags & MM_FDMEM_ZOMBIE) crash();
		}
		obp->mem.mm.flags &= ~MM_MEM_INPROGRESS;
	}
	if(!(flags & MM_FDMEM_CLEANUP)) pthread_mutex_unlock(&fdmem_mutex);
...
mem_map_fd(resmgr_context_t *ctp, PROCESS *prp, mem_map_t *mmapp, OBJECT **object) {
	...
	if(obp == NULL) {
		...
		mempart_id_t mpart_id = mempart_getid(prp, sys_memclass_id);
		obp = object_create(OBJECT_MEM_FD, (void *)mmap_fd, mpart_id);
		if(obp == NULL) {
			// not enough memory
			if(fd != mmap_fd) {
				pthread_mutex_unlock(&fdmem_mutex);
	...
	if(obp->mem.mm.flags & MM_FDMEM_ZOMBIE) {
		OBJECT	**owner;
		OBJECT	*chk;
	...
	obp->mem.mm.flags |= MM_MEM_INPROGRESS;
	obp->mem.mm.flags += MM_MEM_HOLD_INC; // temp kludge
	pthread_mutex_unlock(&fdmem_mutex);
	...
	proc_mux_lock(&obp->mem.mm.mux);
	obp->mem.mm.flags -= MM_MEM_HOLD_INC; // temp kludge
	if((obp->mem.mm.flags & MM_MEM_HOLD_MASK) == 0) {
		obp->mem.mm.flags &= ~MM_MEM_INPROGRESS;
	}
...
memmgr_fd_compact(void) {
	if(!(atomic_set_value(&fdmem_cleanup, MMAPFD_CLEANUP_QUEUED) & MMAPFD_CLEANUP_QUEUED)) {
...
memmgr_fd_cleanup(message_context_t *ctp, int code, unsigned flags, void *handle) {
	...
	if(proc_thread_pool_reserve() == 0) {
		if(atomic_set_value(&fdmem_cleanup, MMAPFD_CLEANUP_QUEUED|MMAPFD_CLEANUP_INPROGRESS) & MMAPFD_CLEANUP_INPROGRESS) {
		...
		for( ;; ) {
			...
			if(!(obp->mem.mm.flags & MM_FDMEM_ZOMBIE)) crash();
			mtime = obp->mem.pm.mtime;
			if(force || (currtime >= mtime)) {
				obp->mem.mm.flags |= MM_FDMEM_CLEANUP;
				*owner = obp->hdr.next;
				if(!object_done(obp)) {
...
__SRCVERSION("memmgr_fd.c $Rev: 156323 $");
