...
vmm_dup(PROCESS *pprp, PROCESS *prp) {
	...
	// Get rid of any mappings established by the vmm_mcreate() - they'll
	// get set up again as we dup the aspace.
	r = vmm_munmap(prp, 0, ~0, 0, mempart_getid(prp, sys_memclass_id));
	if(r != EOK) goto fail1;
	...
	for( ;; ) {
		...
		if(!(pmm->extra_flags & EXTRA_FLAG_NOINHERIT)) {
			r = map_create(&ms, &repl_ms, &adp->map, pmm->start, 
						(pmm->end - pmm->start) + 1, 0, MAP_FIXED);
			if(r != EOK) goto fail1;			
			mm = ms.first;
			...
			por = pmm->obj_ref;
			if(por != NULL) {
				if(((pmm->mmap_flags & (MAP_ANON|MAP_TYPE)) == (MAP_ANON|MAP_PRIVATE)) && !(pmm->mmap_flags & MAP_STACK)) {
					adp->rlimit.data += len;
...
__SRCVERSION("vmm_dup.c $Rev: 156323 $");
