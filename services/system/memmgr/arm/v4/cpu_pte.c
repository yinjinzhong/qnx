	...
	while (mva < end) {
		...
		if ((*pte & ARM_PTE_VALID) == 0) {
			...
			if ((*ptp & ARM_PTP_VALID) == (ARM_PTP_SC & ARM_PTP_VALID)) {
				...
				l2_addr |= ARM_PTP_L2;
				PTP_DOMAIN_SET(l2_addr, data->adp->cpu.domain);
	...
	while (mva < end) {
		...
		if ((*pte & ARM_PTE_VALID) == 0 || (*ptp & ARM_PTP_VALID) == (ARM_PTP_SC & ARM_PTP_VALID)) {
			...
			if (npg == ARM_SCSIZE / __PAGESIZE) {
				 ...
				 */
				opte = l2_to_sc(*pte);
				PTP_DOMAIN_SET(opte, data->adp->cpu.domain);
				*ptp = opte;
				npg = 0;
				while (npg < ARM_SCSIZE / __PAGESIZE) {
					arm_v4_idtlb_addr(mva);
...
cpu_pte_manipulate(struct mm_pte_manipulate *data)
{
	...
	unsigned	dflush = 0;
	int			r;
	PROCESS *	prp = object_from_data(adp, address_cookie);
	...
	if (data->op & PTE_OP_BAD) {
		...
		if (va < USER_SIZE) {
			...
			if (ARM_GBL_PROTECTED(data->shmem_flags)) {
	...
	while (va < end) {
		 ...
		 */
		CRASHCHECK((ptp[0] & ARM_PTP_VALID) != (ARM_PTP_L2 & ARM_PTP_VALID) ||
				   (ptp[1] & ARM_PTP_VALID) != (ARM_PTP_L2 & ARM_PTP_VALID) ||
				   (ptp[2] & ARM_PTP_VALID) != (ARM_PTP_L2 & ARM_PTP_VALID) ||
				   (ptp[3] & ARM_PTP_VALID) != (ARM_PTP_L2 & ARM_PTP_VALID));
...
__SRCVERSION("cpu_pte.c $Rev: 156323 $");
