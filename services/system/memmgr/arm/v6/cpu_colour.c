...
#ifdef	FIXME
		/*
		 * FIXME: need an ARM1136 board to test this with.
		 *        On the MPcore board, we have a physical cache so this
		 *        code should never be executed (we have only 1 colour).
		 */
		colour_operation(func, colour, paddr);
#else
kprintf("paddr: %x colour %x != %x\n", paddr, colour, COLOUR_PA(paddr));
crash();
#endif
	}
}

__SRCVERSION("cpu_colour.c $Rev: 153052 $");
