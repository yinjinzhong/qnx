...
ptzero(paddr_t pa)
{
	pte_t	*ptep = KTOPTEP(ARM_V6_SCRATCH_PTBL);

	*ptep = pa | l2_prot;
	CPU_ZERO_PAGE((void *)ARM_V6_SCRATCH_PTBL, __PAGESIZE, 0);
	*ptep = 0;
...
cpu_pte_manipulate(struct mm_pte_manipulate *data)
{
	...
	while (va < end) {
		...
		CRASHCHECK((ptp[0] & ARM_PTP_VALID) != ARM_PTP_V6_L2 ||
				   (ptp[1] & ARM_PTP_VALID) != ARM_PTP_V6_L2 ||
				   (ptp[2] & ARM_PTP_VALID) != ARM_PTP_V6_L2 ||
				   (ptp[3] & ARM_PTP_VALID) != ARM_PTP_V6_L2);
		...
		do {
			...
			if (npte != opte) {
				if (opte) {
					 ...
					 *
					 * FIXME_v6: do we need to flush the cache?
					 */
					if ((opte & ARM_PTE_C) && (npte & ~PGMASK) != (opte & ~PGMASK)) {
						flags |= PMF_FLUSH_CACHE;
						flags |= arm_cpu->page_flush(va | adp->cpu.asid, 1);
					}
					else {
						flags |= arm_cpu->page_flush(va | adp->cpu.asid, 0);
					}
#else
					/*
