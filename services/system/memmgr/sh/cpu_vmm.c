...
static uintptr_t 						pgszlist[4];

#define PTE_ATTR(pte)			((pte) & (__PAGESIZE-1) )
#define PTE_PERMS(pte)			(PTE_ATTR(pte) & ~(SH_CCN_PTEL_SZ0 | SH_CCN_PTEL_SZ1) )
#define PTE_PGSIZE(pte)			((pte & SH_CCN_PTEL_SZ1)?((pte & SH_CCN_PTEL_SZ0)?MEG(1):KILO(64)):4096)
#define PTE_PGSIZE_TO_ATTR(size)	((size == 4096)?SH_CCN_PTEL_SZ0:(size == MEG(1))?(SH_CCN_PTEL_SZ0 |SH_CCN_PTEL_SZ1):SH_CCN_PTEL_SZ1)


// ptemasks:
// Masks used to find the beginning of a page table entry address range given the pteh value,
// indexed by the size entry (sz bits of the ptel value)
static const unsigned ptemasks[4] = {
	0xfffffc00,		// sz==0 ->  1K pages
	0xfffff000,		// sz==1 ->  4K pages
	0xffff0000,		// sz==2 -> 64K pages
	0xfff00000 		// sz==3 ->  1M pages
};

// ptesizes:
// Size (in bytes) of a page table entry, given the size entry (sz bits of the ptel value)
static const unsigned ptesizes[4] = {
	0x400,			// sz==0 -> 1K
	0x1000,			// sz==1 -> 4K
	0x10000,		// sz==2 -> 64K
	0x100000		// sz==3 -> 1M
...
sh4_update_tlb(uintptr_t vaddr, uint32_t ptel) {
	...
	pteh = (vaddr & ptemasks[SH_CCN_SZ_FROM_PTEL(ptel)]) | (pteh & SH_CCN_PTEH_ASID_M);
...
perm_map_check(uintptr_t badva, unsigned write_fault, unsigned load_tlb) {
	...
	for(i = num_permanent_tlb-1; i >= 0; --i) {
		...
		mask = ptemasks[SH_CCN_SZ_FROM_PTEL(perm->data)];
 ...
 */
void
tlb_flush_va(ADDRESS *adp, uintptr_t start, uintptr_t end) {
	unsigned 	index, sz;
	uint32_t	ptel, pteh;
	uint32_t	mask;
	...
	for(index=0;index<SH4_ITLB_SIZE;index++) {
		...
		// after pte ]"
		if( (SH_CCN_ASID_FROM_PTEH(pteh)== asid) && !( (end < vaddr) || (start > vaddr+ptesizes[sz]-1) ) ) {
...
alloc_pgentry(PROCESS *prp) {
	...
	unsigned			pa_status;
	memsize_t			resv = 0;
	mempart_id_t		mpid = mempart_getid(prp, sys_memclass_id);
	...
	if(pde != NULL) {
		...
		if(PRESENT(pte)) {
			...
			if((check < vaddr) || ((vaddr == data->start) && (check_end > data->end))) {
				...
				tlb_flush_va(data->adp, vaddr, check_end);
	...
	do {
		...
		for( ;; ) {
			...
			if(chk_ptep == NULL) {
				...
				chk_pte = *chk_ptep;
				if(PTE_PERMS(pte) != PTE_PERMS(chk_pte)) break;
				if((REAL_ADDR(chk_pte) + PTE_PGSIZE(chk_pte)) != REAL_ADDR(pte)) break;
			}
			run_last = data->start;
			data->start += PTE_PGSIZE(pte);
...
cpu_pte_manipulate(struct mm_pte_manipulate *data) {
	...
	if(data->op & PTE_OP_BAD) {
		...
		bits = SH_CCN_PTEL_SZ0;  // set 4K page size
		//RUSH2: This should be a __cpu_flags bit so we don't have
		//RUSH2: continually add new PVR values here.
		switch(SH4_PVR_FAM(in32(SH_MMR_PVR))) {
		case SH4_PVR_SH4A:
			// SH4A ignores special field
			break;
		default:
	...
	for( ;; ) {
		...
		if(pde == NULL) {
			...
			pde = alloc_pgentry(object_from_data(adp, address_cookie));
			if(pde == NULL) {
				if(need_flush) {
...
cpu_vmm_vaddrinfo(PROCESS *prp, uintptr_t vaddr, paddr_t *paddrp, size_t *lenp) {
	...
	*paddrp = REAL_ADDR(pte) | offset;
	if(lenp != NULL) {
		unsigned pgsize;

		switch(pte & SH_CCN_PTEL_SZ_M) {
		case SH_CCN_PTEL_SZ1:
			pgsize = KILO(64);
			break;
		case SH_CCN_PTEL_SZ1 | SH_CCN_PTEL_SZ0:
			pgsize = MEG(1);
			break;
		default:
			pgsize = __PAGESIZE;
