...
extern int		memobj_pmem_del(OBJECT *, off64_t, uint64_t);
...
#ifdef CHECK_ALIGN	
	#define CHECK_PTE_ALIGNMENT(d)					\
		if(ADDR_OFFSET((d)->start) != 0) crash();	\
		if(ADDR_OFFSET((d)->end+1) != 0) crash();	\
		if(((d)->op & PTE_OP_MAP) && ADDR_OFFSET((d)->paddr) != 0) crash();
#else
	#define CHECK_PTE_ALIGNMENT(d)
#endif
...
/* __SRCVERSION("vmm.h,v $Rev: 157184 $"); */
