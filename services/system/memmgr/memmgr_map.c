...
check_again:	
	...
	mpart_id = mempart_getid(prp, sys_memclass_id);
	if((object == NULL) && ((msg->i.flags & (MAP_ANON|MAP_TYPE)) == (MAP_ANON|MAP_SHARED))) {
		...
		object = object_create(OBJECT_MEM_SHARED, msg, mpart_id);
		if(object == NULL) {
			return ENOMEM;
		}
	...
	if(proc_thread_pool_reserve() != 0) {
		if(object != NULL) {
			if(object->hdr.type == OBJECT_MEM_FD) {
				fdmem_done_prep(object);
			}
	...
	if (object != NULL)	mpart_id = object->hdr.mpid;
	for( ;; ) {
		...
		// the object lock first, but we turn on the MM_MEM_INPROGRESS
		// flag so that no one deletes the object on us while we've
		// got it unlocked.
		if(object != NULL) {
			object->mem.mm.flags |= MM_MEM_INPROGRESS;
			object->mem.mm.flags += MM_MEM_HOLD_INC; // temp kludge
			proc_mux_unlock(&object->mem.mm.mux);
		}
		...
		if(object != NULL) {
			proc_mux_lock(&object->mem.mm.mux);
			object->mem.mm.flags -= MM_MEM_HOLD_INC; // temp kludge
			if((object->mem.mm.flags & MM_MEM_HOLD_MASK) == 0) {
				object->mem.mm.flags &= ~MM_MEM_INPROGRESS;
			}
		}
		if(!try_again) break;
...
__SRCVERSION("memmgr_map.c $Rev: 156323 $");
