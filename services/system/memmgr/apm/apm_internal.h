...
*/
#define ISSET_MEMPART_POLICY_KEY(v)			((v) == (apm_bool_t)(uintptr_t)&(v))
#define SET_MEMPART_POLICY_KEY(v, p)		(((v) == (p)) ? ((v) = (apm_bool_t)((uintptr_t)&(v))):0)
 ...
 * representation of the policy and the external (apm_bool_t) representation of the
 ...
 * HEAP_PRIME macros
 * 
 * These macros will encode a heap prime size value from 0x1000 to 0xF000 
*/
#define HEAP_PRIME_ENCODE(n)	((((unsigned)(n)) & 0xF000U) << 16)
#define HEAP_PRIME_EXTRACT(f)	((((unsigned)(f)) & mempart_flags_RESERVED) >> 16)
...
#define __dump_list(lp) \
		do { \
			while ((mpart_qnode_t *)lp != NULL) { \
				kprintf("%p(%x) -> ", (mpart_qnode_t *)lp, *((unsigned int *)&lp[1])); \
				(mpart_qnode_t *)lp = ((mpart_qnode_t *)lp)->next; \
			} \
			kprintf("@\n"); \
		} while(0)
