...
proc_lock_owner_mark(PROCESS *prp) {
	...
	if(adp->fault_owner == 0) {
		adp->fault_owner = MAKE_OWNER(getpid(), pthread_self());
	}
...
proc_lock_owner_check(PROCESS *prp, pid_t pid, unsigned tid) {
	return prp->memory->fault_owner == MAKE_OWNER(pid, tid);
}

__SRCVERSION("memmgr_support.c $Rev: 153052 $");
