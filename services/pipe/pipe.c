...
/*
 *  Conditionally call iofunc_notify_remove() for the pipe; problem is
 *  that early libc versions were buggy and did scoid-only matching and
 *  remove too many entries if called from closedup time; can only call
 *  if newer libc in place (which does scoid/coid matching in a new
 *  iofunc_notify_remove_strict() routine).  Using a "weak alias"
 *  mechanism to do this does not work on older QNX "ld" versions; a
 *  static link cannot "dlopen()", so assume linked against new libc.
 */
void notify_remove(resmgr_context_t *ctp, pipe_t *pipe)
{
static int	strict = !0;
void		*dll;

	if (pipe == NULL) {
		if ((dll = dlopen(NULL, RTLD_WORLD)) != NULL) {
			if (dlsym(dll, "iofunc_notify_remove_strict") == NULL)
				strict = 0;
			dlclose(dll);
		}
	}
...
int			opt;
	...
	while ((opt = getopt(argc, argv, ":1a:Ps:")) != -1 || optind < argc) {
		switch (opt) {
		case '1':
			SelWrite1 = 0;
			break;
