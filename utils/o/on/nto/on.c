...
int wait_for( int argc, char *argv[] )
{
    ...
    count = 0;
    while( ( sample_ms * count ) < timeout_ms ) 
    {
...
int main(int argc, char *argv[]) {
	...
	while((i = getopt(argc, argv, "Ddhsl:f:n:p:u:t:C:R:X:w:W:")) != -1) {
		switch(i) {
		case 'D':	/* debug */
		...
		case 'w':
			while(device(optarg, 0) == -1 && (errno == ENOENT || errno == ENOSYS || errno == EAGAIN || errno == ENXIO || (errno >= ENETDOWN && errno <= EHOSTUNREACH)) && count--) {
				delay(100);
			}
