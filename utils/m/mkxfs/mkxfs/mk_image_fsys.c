...
void
copy_elf(int fd, FILE *dst_fp, struct file_entry *fip, int dohdr) {
	Elf32_Ehdr			ehdr;
...
do_load:			
			...
			if(segsize != 0) {
				...
				if(phdr->p_offset == 0) {
					...
					// going to write out the new ehdr separately.
					//
				...
				if(congruence > 0) {
					...
					adjusted_off = off - sinfo[i].hdr_adjust;
					disp = (EHost32(fip, phdr->p_vaddr) - fip->run_offset) & congruence;
...
ifs_make_fsys(FILE *dst_fp, struct file_entry *list, char *mountpoint, char *destname) {
	...
	iwrite(mountpoint, hsize - n, dst_fp, "Image-header");
...
__SRCVERSION("mk_image_fsys.c $Rev: 155019 $");
