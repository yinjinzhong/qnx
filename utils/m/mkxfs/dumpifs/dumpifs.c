...
%C	[-mvxb -u file] [-f file] image_file_system_file [files]
...
int main(int argc, char *argv[]) {
	...
	while((c = getopt(argc, argv, "f:d:mvxbu:")) != -1) {
		switch(c) {
...
void process(const char *file, FILE *fp) {
	...
	if((ipos = find(fp, ihdr.signature, sizeof ihdr.signature, 0)) == -1) {
		...
		if((shdr.flags1 & STARTUP_HDR_FLAGS1_COMPRESS_MASK) != 0) {
            ...
            #ifdef __MINGW32__ 
            /* Explicitly set the file pointer to the beginning of the compressed data. 
               This must be done under win32 because the cygwin file ops don't modify the file 
               pointer in the same manner as the QNX file ops. This is a fix for PR#21896. 
            */
            {
                fpos_t   pos;    /* to manually set the file pointer location (win32) */
				
                pos = spos + shdr.startup_size;
                fsetpos(fp, &pos);
            }
            #endif
...
#ifdef __QNXNTO__
__SRCVERSION("dumpifs.c $Rev: 153624 $");
#endif
