...
void shutdown(int type,int flags)
{
	...
	pid_t me = getpid(),pid;
	ProcessInfo_t *pidvec,*pip;
	struct sched_param param;
	DisplayData_t ddata;
	size_t pid_count;
	char const *proc_dir = "/proc";
	...
	if (!(flags & FLAG_NO_DAEMON)) {
		/* Make me a daemon */
		procmgr_daemon(0,PROCMGR_DAEMON_NODEVNULL | PROCMGR_DAEMON_NOCLOSE);
	}
	...
	if(!(pidvec = malloc(pid_count * sizeof(*pidvec))))
	{
		closedir(dir);
	...
	for(rewinddir(dir),pid_count = 0,pip = pidvec;dirent = readdir(dir);)
	{
		...
		if(pid > 1 && pid != me)
		{
			snprintf(buf,sizeof(buf),"%s/%s/as",proc_dir,dirent->d_name);
