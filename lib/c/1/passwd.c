...
#if 0			//Not in header files, but we might need it someday
struct passwd *getpwent_r(struct passwd *result, char *buffer, int bufsize) {
	...
	id.uid = -1;
	dbase_lookup(&request, id, 0, (union record **)&lpwd);
	errno = saved;
	return((lpwd && lpwd->pw_name) ? result : NULL);
}
#endif
...
__SRCVERSION("passwd.c $Rev: 153052 $");
