...
unsigned __malloc_mmap_flags=MAP_ANON;
...
static void get_environ_vars()
{
  ...
  p = getenv("MALLOC_MEMORY_BANDCONFIG");
	if (p) {
		...
		if ((__dynamic_Bands) && (__dynamic_nband)) {
  ...
  if (p) {
    ...
    if (sz > 0) {
      ...
      __core_cache_max_num = -1;
      __core_cache_max_sz = -1;
		}
  }
...
__SRCVERSION("dlist.c $Rev: 156243 $");
