...
int thread_pool_control(thread_pool_t *pool, thread_pool_attr_t *attr, 
	...
	_thread_pool_attr_normalise(attr);
	...
	*/
	...
	while(!done) {
		if((pool->waiting + props->newthreads) < pool->pool_attr.lo_water) {
		...
		} else if((pool->created > pool->pool_attr.maximum) ||
			...
			count = __max((int)(pool->created - pool->pool_attr.maximum), 0);
		...
		if(!(flags & THREAD_POOL_CONTROL_NONBLOCK)) {
			atomic_set(&pool->flags, POOL_FLAG_CHANGING);
			pthread_sleepon_lock();
			while(!(lower <= pool->created && pool->created <= upper)) {
				pthread_sleepon_wait(pool);
			}
			pthread_sleepon_unlock();
			atomic_clr(&pool->flags, POOL_FLAG_CHANGING);
...
__SRCVERSION("thread_pool_ctrl.c $Rev: 153052 $");
