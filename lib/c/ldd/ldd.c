...
static void *static_memcpy(void *dst, const void *src, size_t nbytes) {
	char			*d = dst;
	const char		*s = src;
...
static int static_strcmp( const char *s1, const char *s2 ) {
	unsigned char	c1;
	unsigned char	c2;
	int				diff;
...
static void vector_decode(unsigned long *dst, int size, const void *src, uint32_t offset) {
	...
	static_memset(dst, -1, size * sizeof *dst);
	while (s->key) {
...
static const Elf32_Sym *hashed_lookup(const char *sym, unsigned long h, const struct object *obj) {
	...
	if (obj->symbols && obj->hash) {
		...
		do {
			if (!static_strcmp(sym, obj->strings + obj->symbols[hv].st_name)) {
...
if(getenv_ne("DL_DEBUG")) {
		fprintf(stderr, "load_elf32: mmaped, addr %p %p vaddr %p\n", 
	...
	Elf32_Dyn *dyn;
		...
		for ( dyn = (Elf32_Dyn *)obj->dynamic; dyn->d_tag != 0; dyn++ )
			if ( dyn->d_tag == DT_TEXTREL ) {
				...
				if(getenv_ne("DL_DEBUG")) {
					fprintf(stderr, "load_elf32: found DT_TEXTREL, mapping a private copy of text sections!\n");
				}
	...
	if(getenv_ne("DL_DEBUG")) {
		fprintf(stderr, "load_elf32: loaded lib at addr %x(text) %x(data)\n", 
...
static int find_file(const char *name, const char *libpath, const char *rpath, const char **fullpath) {
	 ...
	 * DT_RPATH, LD_LIBRARY_PATH, /usr/lib.
	 */  
	//PR 7654 and Unix98 say we look at the whole path for a slash character 
	for(fd = 0; name[fd] != '/' && name[fd] != '\0'; fd++) { ; }

	if (name[fd] != '/') {
		if ((rpath == NULL || searchpath(name, rpath, R_OK, pathname, sizeof pathname) == -1) &&
				(_is_setuid ? _is_setuid : searchpath(name, libpath, R_OK, pathname, sizeof pathname) == -1) &&
				searchpath(name, _system_libpath, R_OK, pathname, sizeof pathname) == -1) {
			return -1;
		}
		name = pathname;
	}
...
static int load_object(struct object *obj, const char *name, const char *libpath, const char *rpath,
	...
	if(getenv_ne("DL_DEBUG")) {
		fprintf(stderr, "load_object: attempt load of %s\n", (name) ? name : ""); 
	}
	...
	dynamic_decode(obj);
	if(!obj->name) { 
		obj->name = strdup(name_only(name));
...
static uintptr_t LDD_C_USEAGE_ATTRIBUTE ldd (void *frame) {
	...
	static_memset(&self, 0, sizeof self);
	...
	static_memset(&other, 0, sizeof other);
	...
	o->object->flags |= OBJFLAG_INIT | OBJFLAG_FINI | OBJFLAG_EXECUTABLE;
	o->root = &objects;
	...
	*_tlop = (int)getenv_ne("LD_TLO");
	...
	if(ld_preload) {
		...
		while(needed && *needed) {
			...
			if (!(obj = find_object(&objects, name_only(needed), NULL))) {
				...
				static_memset(obj, 0, sizeof *obj);
	...
	stuff_path_and_time(&self._link_map, strdup(_interp));
	stuff_path_and_time(&other._link_map, strdup(argv[0]));
				...
				if (!(obj = find_object(&objects, needed, NULL))) {
					if (!strcmp(needed, self.name)) {
						...
						if (load_object(obj, o->object->strings + 
							...
							error("Could not find library ", o->object->strings + dyn->d_un.d_val);
						} 
					}
					obj->refcount++;
					l = _alloca(sizeof *l);
	...
	list_forward(&objects, o) {
		...
		static_memcpy(obj, o->object, sizeof *obj);
...
static void _dl_free_object(struct object *obj) {
	...
	if (obj->_link_map.l_path) {
		free ((void *)obj->_link_map.l_path);
	}
	free(obj);
}
...
#pragma disable_message(201);
	...
	libpath = getenv_ne(LIBPATHENV);	/* @@@ don't do this if SETUID */
	 ...
	 */
	obj = NULL;
	static_memset(&stkobj, 0x00, sizeof stkobj);
	...
	if (obj == NULL) {
		...
		static_memcpy(obj, &stkobj, sizeof stkobj);
	 ...
	 * only RTLD_GROUP.
	 */
	if(!list_iszero(&_dl_start_objects) && !list_isempty(&_dl_start_objects) && (mode & RTLD_WORLD)) {
		int		first = 1;
	 ...
	 */
	if(debug){
		printf("Library loaded; type \'add-sym %s %p\' in gdb to load symbols\n",
	...
	if(mode & RTLD_GROUP) {
		list_forward(handle, olist) {
			...
			*/
			if(olist->flags & OBJLFLAG_REMOVE) {
				continue;
			}
...
void *dlsym(void *handle, const char *name) {
	...
	if(getenv_ne("DL_DEBUG")) {
		debug = 1;
		fprintf(stderr, "dlsym(%p,%s)=", handle, name);
	}
                      ...
                      unsigned flags) 
{
	...
	node = malloc(sizeof(Ldd_Eh_Node_t));
	if (node == NULL) 
		return(NULL);
	node->eh = eh;
