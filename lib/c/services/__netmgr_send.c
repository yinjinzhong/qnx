...
int __netmgr_send(void *smsg1, int ssize1, const void *smsg2, int ssize2, void *rmsg, int rsize) {
	int					coid;
	int					status;
	static int			netmgr_coid = -1;
	iov_t				iov[2];
	int					parts = 1;
	...
	if(  (coid = netmgr_coid) == -1
	  ...
	  && ((errno == EBADF) || (errno == ENOSYS)))) {
		...
		if((coid = _netmgr_connect(_NTO_SIDE_CHANNEL, "/dev/netmgr", 0, O_RDONLY, SH_DENYNO,
					_IO_CONNECT_OPEN, 0, _IO_FLAG_RD | _IO_FLAG_WR,
					0, 0, 0, 0, 0, 0, 0)) == -1) {
			errno = ENOTSUP;
			return -1;
		}
		...
		if((status = MsgSendvs(coid, iov, parts, rmsg, rsize)) == -1) {
...
__SRCVERSION("__netmgr_send.c $Rev: 153052 $");
