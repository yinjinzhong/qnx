...
#define __va_start_stdarg(AP,LASTARG) \
...
__extension__ (*({							\
  ...
  if (__va_float_p (TYPE) && (AP)->fpr < 8)				\
    ...
    {									\
      ...
      (AP)->overflow_arg_area = (void *)__ptr + __va_size (TYPE) * sizeof (long);	\
    }									\
  else									\
    {									\
      __ptr = (TYPE *) (void *) ((AP)->overflow_arg_area);		\
...
#define __va_dcl
#define __va_start_vararg(AP) \
  (__builtin_next_arg (__alist), __va_start_common (AP, 1))
...
extern 
    ...
    "C"
#endif
void __va_arg_type_violation(void) __attribute__((__noreturn__));
  ...
  if (__va_vector_p (TYPE))						   \
    {									   \
      ...
      __va_overflow(AP) += __va_size (TYPE) * sizeof (long);		   \
									   ...
									   \
  else if (__va_float_p (TYPE) && sizeof (TYPE) < 16)			   \
    {									   \
...
#define __va_alist        __va_1st_arg
#define __va_dcl	  void *__va_1st_arg; ...
#define __va_start_vararg(AP) __va_start_common (AP, 1)
