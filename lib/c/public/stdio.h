...
extern int      vfscanf( _CSTD FILE *__fp, const char *__format, __va_list __arg ) __attribute__((format (scanf, 2, 0)));
...
inline int putc(int _C, FILE *_Str)
	{	/* put a character */
	return (((_Multi_threaded || _Str->_Next >= _Str->_Wend)
		? fputc(_C, _Str) : (*_Str->_Next++ = _C))); }

inline int putchar(int _C)
	{	/* put a character to stdout */
	return (((_Multi_threaded || _Files[1]->_Next >= _Files[1]->_Wend)
...
__END_DECLS
#if defined(__WATCOMC__) && (defined(__SW_BD) || defined(__SW_BM))
#define getc(__fp)          (fgetc)(__fp)
#define putc(__c,__fp)      (fputc)(__c,__fp)
#define getchar()           (fgetc)(_Files[0])
#define putchar(__c)	    (fputc)(__c,_Files[1])
#else
#define getc(__fp) \
	((_Multi_threaded || (__fp)->_Next >= (__fp)->_Rend) ? \
	(fgetc)(__fp) : *(__fp)->_Next++)
#define getchar() \
	((_Multi_threaded || _Files[0]->_Next >= _Files[0]->_Rend) ? \
	(fgetc)(_Files[0]) : *_Files[0]->_Next++)
...
#define putchar(__c) \
	...
	(fputc)(__c, _Files[1]) : (*_Files[1]->_Next++ = __c))
#endif

#if defined(__EXT_POSIX1_199506)
#if !defined(__WATCOMC__) || !(defined(__SW_BD) || defined(__SW_BM))
#define getc_unlocked(__fp) \
	((__fp)->_Next >= (__fp)->_Rend ? \
	(fgetc)(__fp) : *(__fp)->_Next++)
#define getchar_unlocked() \
	(_Files[0]->_Next >= _Files[0]->_Rend ? \
	(fgetc)(_Files[0]) : *_Files[0]->_Next++)
#define putc_unlocked(__c, __fp) \
	((__fp)->_Next >= (__fp)->_Wend ? \
	(fputc)(__c, __fp) : (*(__fp)->_Next++ = __c))
#define putchar_unlocked(__c) \
...
/* __SRCVERSION("stdio.h $Rev: 154597 $"); */
