...
#define FD_SET(fd, p)   ((p)->fds_bits[(fd)/(int)(sizeof(int)*CHAR_BIT)] |= \
                (1 << ((fd) % (int)((sizeof(int))*CHAR_BIT))))

#define FD_CLR(fd, p)   ((p)->fds_bits[(fd)/(int)(sizeof(int)*CHAR_BIT)] &= \
                ~(1 << ((fd) % (int)(sizeof(int)*CHAR_BIT))))

#define FD_ISSET(fd, p) ((p)->fds_bits[(fd)/(int)(sizeof(int)*CHAR_BIT)] & \
                (1 << ((fd) % (int)(sizeof(int)*CHAR_BIT))))
...
/* __SRCVERSION("select.h $Rev: 153052 $"); */
