...
inline int bcmp(const void *_Str1, const void *_Str2, size_t _Num)
  { return memcmp(_Str1, _Str2, _Num); }
inline void bcopy(const void *_Src, void *_Dst, size_t _Num)
  { (void)memmove(_Dst, _Src, _Num); return; }
inline void bzero(void *_Dst, size_t _Num)
  { (void)memset(_Dst, 0, _Num); return; }
inline const char *index(const char *_Src, int _Cnt)
  { return strchr(_Src, _Cnt); }
inline const char *rindex(const char *_Src, int _Cnt)
  { return strrchr(_Src, _Cnt); }
...
/* __SRCVERSION("strings.h $Rev: 153052 $"); */
