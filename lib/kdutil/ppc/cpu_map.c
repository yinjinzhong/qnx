...
cpu_map(paddr64_t paddr, unsigned size, unsigned prot, int colour, unsigned *mapped_len) {
	...
	case PPC_FAMILY_booke:	
		...
		temp_tlb.v = 1;
		// tlbno==0, entryno==0 is a 256M covering entry with TS==0.
		// tlbno==0, entryno==1 is a 256M covering entry with TS==1.
		...
		bke_tlb_write(0, (get_msr() & PPC_MSR_IS) ? 0 : 1, &temp_tlb);
		break;
	case PPC_FAMILY_900:
		mapping_size = 0x1000;
		base = paddr & ~0xfff;
...
cpu_unmap(void *p, unsigned size) {
	...
	case PPC_FAMILY_booke:	
		...
		bke_tlb_write(0, i, &state->u.ppcbke.tlb[0].entry[i]);
