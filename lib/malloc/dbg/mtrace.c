	...
	if (file != NULL) {
		...
		if (!justline) {
...

			{
				...
				buf[len++] = ' ';
				tostring(&buf[len], (ulong_t)(__tls()->__pid),10,B_DEC,' ');
				len += 10;
				buf[len++] = ' ';
				tostring(&buf[len], 
				(ulong_t)(dd ? dd->tid :__tls()->__tid),5,B_DEC,' ');
				len += 5;
				buf[len++] = ' ';
...
mtrace_malloc(const char *file, int line, size_t size)
{
	...
	if (cptr != NULL && MALLOC_TRACE_FCHECK(size)) {
		if (!__check_ctid()) {
			...
			(void) tostring(&buf[len], (ulong_t)(__tls()->__pid),10,B_DEC,' ');
			len += 10;
			buf[len++] = ' ';

			(void) tostring(&buf[len], (ulong_t)(__tls()->__tid),5,B_DEC,' ');
			len += 5;
			buf[len++] = ' ';
...
{ 
    ...
    if (__malloc_trace_fd >= 0) {
		...
		*s++ = ' ';
		s += tostring(s, (int)__tls()->__pid,10,B_DEC,' ');
		*s++ = '\n';
		*s++ = '\0';
...
{ 
	int fd=-1;
	ENTER();
	if (__malloc_trace_filename != NULL || __malloc_trace_filename[0] != '\0') {
