...
#ifndef lint
static char rcs_header[] = "$Id: malloc_g.c 153052 2007-11-02 21:10:56Z coreos $";
#endif
#endif
...
void __dbgm_print_snapshots(int fd) {
	...
	/* Get the malloc usage events.  */
	{
		...
		*s++ = 'E';
		*s++ = ':';
		s += sprintf(s, "%u:%u:%u:%u:0x%lx\n", overheadmem, usemem, freemem, __tls()->__pid, (ulong_t)timestamp);
		write(fd, buf, strlen(buf));
	}
...
{
	//int line = (int)__builtin_return_address(0);
	int line;
	__my_builtin_return_address(line, 0);
	if (fd < 0)
		return(-1);
...
debug_malloc(const char *file, int line, size_t size)
{
	...
	MALLOC_GETBT(__cdbt);
	/* The MALLOC_GETBT is done with the __builtin_xxxx_address calls
	 * unfortunately it sometimes fails so we fallback on another scheme
	 */
	if (__malloc_bt_depth > 1 && (__cdbt[0] == NULL || __cdbt[1] == NULL)) {
		void *array[__malloc_bt_depth];
		int size, i;
		size = backtrace(array, __malloc_bt_depth, line);
		for (i = 0; i < size; ++i) {
			__cdbt[i] = (unsigned int *)array[i];
		}
		if (i < CALLERPCDEPTH) {
			__cdbt[i] = NULL;
		}
...
posix_memalign(void **ptr, size_t alignment, size_t size)
{
	...
	void *p;
	unsigned int *pc;
	__my_builtin_return_address((int)pc, 0);
	...
	p = memalign_pc(alignment, size, pc);
	if (p != NULL) {
		*ptr = p;
	} else {
		return ENOMEM;
              ...
              int line, const void *link, int type)
{
	...
	//Indicate that this particular event reporting is beginning
	{
		...
		*s++ = ' ';
		s += tostring(s,(ulong_t)__tls()->__pid,10,10,' ');
		*s++ = ' ';
		s += tostring(s,(ulong_t)__tls()->__tid,5,10,' ');
		*s++ = '\n';
		*s = '\0';
	...
	if (mptr != NULL && MAGIC_MATCH(mptr)) {
		...
		*s++ = ':';
		...
		if (mptr->dbg != NULL) {
			...
			*s++ = ':';
			s += tostring(s,(ulong_t)(__tls()->__pid),10,B_DEC,'0');
			*s++ = ':';
			...
			*s++ = ':';
			s += tostring(s,__tls()->__pid,10,B_DEC,'0');
			*s++ = ':';
...
//		s += tostring64(s,(uint64_t)mptr->dbg->ts,18,B_HEX,'0');
		...
		*s++ = ':';
		 ...
		 * Put the file info or the address info in.
	 	 */
		if((mptr->file != NULL) && (mptr->file[0] != '\0')) {
			t = (char *) mptr->file;
			COPY(s,t,errbuf,ERRBUFSIZE);
		...
		*s++ = ':';
		s += tostring(s,(ulong_t)(__tls()->__pid),10,B_DEC,'0');
		*s++ = ':';
		s += tostring(s,(ulong_t)(__tls()->__tid),5,B_DEC,'0');
		*s++ = ':';
    	s += tostring64(s,(uint64_t)timestamp,18,B_HEX,'0');
		*s++ = ':';
	}
...
malloc_dump_info_block(const void *vlink, int id)
{
	...
	char		* s;
	char		* t;
	DebugInfo_t	* mptr = &((Dhead *)vlink)->d_debug;
...
malloc_dump_line(int fd, const char *file, unsigned long line, const char *prefix)
{
	...
	*s++ = ' ';
	(void) tostring(s, (ulong_t)(__tls()->__pid),10,B_DEC,' ');
	s += 10;
	*s++ = ' ';
	(void) tostring(s, (ulong_t)(__tls()->__tid),5,B_DEC,' ');
	s += 5;
	*s++ = '\n';
	*s = '\0';
	writeFully(fd,buf,s-buf);
