...
find_malloc_ptr(const void *ptr, arena_range_t *range)
{
    ...
    MALLOC_CHECK_INIT();
	     ...
	     */
            for (bp = band->alist; rtn == NULL && bp; bp = bp->next)
            {
	        int esize = bp->nbpe + SB_OVERHEAD();

	        if ((ulong_t)ptr >= (ulong_t)(bp+1)
	            && (ulong_t)ptr < (ulong_t)(bp+1) + esize*band->nalloc)
	        {
	            /*
	             * Calculate the small buffer position
	             */
	            int diff = (ulong_t)ptr - (ulong_t)(bp+1);
	            int i = diff / esize;
	            Dhead *dh = (Dhead *)((char *)(bp + 1) + i*esize);
	     ...
	     */
            for (bp = band->dlist; rtn == NULL && bp; bp = bp->next)
            {
	        int esize = bp->nbpe + SB_OVERHEAD();
 ...
 */
...
#endif
	        {
	                    ...
	                    flink = flink->f_next)
	            {
	                ...
	                Block	*b = (Block *)data;
                        /*
                         * Okay, see if it's a block.  If so, we have
                         * a small buffer in the block -- free or allocated
                         */
	                if (b->magic == BLOCK_MAGIC 
	                    && find_block_in_band(b, b->band)) {
	                    /*
	                     * Calculate the small buffer position
	                     */
	                    int esize = b->nbpe + SB_OVERHEAD();
	                    int diff = ((char *)ptr - (char *)(b + 1));
	                    int nb = diff / esize;
	                    Dhead *dh = (Dhead *)((char *)(b + 1) + nb*esize);

	                    range->r_start = (char *)(b + 1);
	                    range->r_end = range->r_start + esize*b->band->nalloc; 
	                    range->r_type = RANGE_BLOCK;
	                    range->un.r_block = b;

	                    /* we found a small buffer */
	                    rtn = (void *)dh;
	                }
	            }
#endif
