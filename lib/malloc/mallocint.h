 ...
 */
/*
 * $Id: mallocint.h 153052 2007-11-02 21:10:56Z coreos $
 */
...
#if defined(__MIPS__) || defined(__ARM__) || defined(__SH__) 
#define __my_builtin_return_address(___line, N) \
    ___line = (int)__builtin_return_address(0); 

#define __my_builtin_return_address_n(___line, N, ___prev) \
    ___line = (int)__builtin_return_address(0); 
#else
#define __my_builtin_return_address(___line, N) \
  if (N < 0) \
...
#define __my_builtin_return_address_n(___line, N, __prev) \
   if (N < 0) \
    ___line = 0; \
  if (N == 0) \
    ___line = (int)__builtin_return_address(0); \
  else  { \
    if ((__prev == 0) || ((int)__prev == (int)__tls()->__exitfunc)) \
      ___line = 0; \
    else { \
      void *__frame; \
      __frame = __builtin_frame_address(N); \
      if (__frame == 0) \
        ___line = 0; \
      else \
        ___line = (int)__builtin_return_address(N); \
        if ((int)___line < (int)__tls()->__stackaddr) \
          ___line = 0; \
    } \
  }
...
#define MALLOC_PTR_FILL_DEBUG(__dd, __ptr) \
	{ \
		__dd = __malloc_add_dbg_info(__dd, (GET_CPU()+1), \
                  __tls()->__tid, (MYCLOCKCYCLES()), __ptr); \
	}
