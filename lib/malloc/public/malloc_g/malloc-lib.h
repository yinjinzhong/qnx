...
extern char  * morecore(unsigned nbytes, int fixed, int *rsize);
...
#define __getmem(nbytes) \
  mmap(0, (nbytes), PROT_READ|PROT_WRITE, MAP_ANON, -1, 0)

#define __putmem(cp,nbytes) \
  munmap((cp),(nbytes))
